...
/**
 * Ratuls_ACT_Logger  v2.4
 *
 * v2.4 fixes:
 *
 *   FIX DASHBOARD-STATIC — log() debug_mode gate swallowed all event data.
 *     log() opened with:
 *       if ( ! get_option( 'ratuls_act_debug_mode', 0 ) ) { return; }
 *     ratuls_act_debug_mode defaults to 0, so every Logger::log() call
 *     from Meta::send(), TikTok::send(), and Google::send() silently
 *     returned without writing anything. ratuls_act_debug_log stayed
 ...
 *
 * v2.1 changes:
 *   - log() fires do_action('ratuls_act_event_logged') after writing.
 *
 * v2.0 changes:
...
class Ratuls_ACT_Logger {
	...
	const OPTION_KEY  = 'ratuls_act_debug_log';
	...
	public static function log(
		...
		if ( ! $force_write && ! get_option( 'ratuls_act_debug_mode', 0 ) ) {
			return;
		}
		 ...
		 * @param array  $emq
		 */
		do_action( 'ratuls_act_event_logged', $platform, $event_type, $order_id, $status, $emq );
	}
	...
	private static function append_entry( array $entry ): void {
		...
		if ( ! $result && $size_mb > 3.0 ) {
			error_log( 'Ratuls_ACT: Debug log update failed. Size: ' . $size_mb . 'MB. Consider clearing debug log.' );
		}
	}
}
