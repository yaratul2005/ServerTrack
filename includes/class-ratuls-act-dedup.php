 ...
 *     Ratuls_ACT_Source_WooCommerce (v3.3.1) calls
 *     Ratuls_ACT_Dedup::already_sent( $key, $platform ) in multiple places:
 ...
 *     "Call to undefined method Ratuls_ACT_Dedup::already_sent()".
 *
 * Changes in v2.3:
...
class Ratuls_ACT_Dedup {
    ...
    const OPTIONS_PREFIX = 'ratuls_act_dedup_';
    ...
    private static function is_hpos(): bool {
        ...
        if (
            ...
            try {
                ...
                error_log( 'Ratuls_ACT: HPOS detection failed - ' . $e->getMessage() );
                if ( class_exists( 'Ratuls_ACT_Logger' ) ) {
                    Ratuls_ACT_Logger::error( 'HPOS detection failed', [ 'exception' => $e->getMessage() ] );
                }
                self::$hpos_enabled = false;
            }
        }
    ...
    public static function get_event_id( int $order_id ): string {
        $event_id = self::get_meta( $order_id, '_ratuls_act_event_id' );
    ...
    public static function store_event_id( int $order_id, string $event_id ): void {
        self::update_meta( $order_id, '_ratuls_act_event_id', $event_id );
    }
    ...
    public static function mark_as_sent( int $order_id, string $platform, bool $save = true ): void {
        $sent = self::get_meta( $order_id, '_ratuls_act_server_sent' );
        if ( ! is_array( $sent ) ) {
            $sent = [];
        }
        ...
        $sent[] = $platform;
        self::update_meta( $order_id, '_ratuls_act_server_sent', $sent, $save );
    }
     ...
     *
     * Ratuls_ACT_Source_WooCommerce calls already_sent() with two different
     * $key types:
     ...
     * BUG-FIX (v2.5): Added to support Ratuls_ACT_Core::dispatch_to_platforms() marking logic.
     *
     * @param int|string $key       Order ID (int) or string dedup key
     * @param string     $platform  'meta' | 'google' | 'tiktok'
     */
     ...
     *
     * Used by: wp ratuls-act test-purchase <order_id>
     *
     * @param int $order_id  WooCommerce order ID.
     */
    public static function reset_for_order( int $order_id ): void {
        self::delete_meta( $order_id, '_ratuls_act_event_id' );
        self::delete_meta( $order_id, '_ratuls_act_server_sent' );
    }
