<?php
/**
 * Ratul_ACT — Offline Conversion Uploader  v3.2
 *
...
class Ratul_ACT_OfflineConversion {
	...
	public static function init(): void {
		add_action( 'ratul_act_offline_upload_batch', [ __CLASS__, 'run_scheduled_upload' ] );
	...
	public static function maybe_queue_order( int $order_id, WC_Order $order ): void {
		...
		if ( Ratul_ACT_Dedup::exists( $key ) ) {
			return;
		}

		Ratul_ACT_Dedup::set( $key );

		wp_schedule_single_event(
			time() + 300,
			'ratul_act_offline_upload_batch',
			[ [ 'order_id' => $order_id ] ]
		);
	}
	...
	private static function build_event_payload( WC_Abstract_Order $order, string $platform ): array {
		...
		if ( $email ) {
			$user_data['em'] = Ratul_ACT_Hasher::hash_email( $email );
		}
		...
		if ( $phone ) {
			$user_data['ph'] = Ratul_ACT_Hasher::hash_phone( $phone, $country );
		}

		// Name fields
		$first_name = $order->get_billing_first_name();
		if ( $first_name ) {
			$user_data['fn'] = Ratul_ACT_Hasher::hash_name( $first_name );
		}
		$last_name = $order->get_billing_last_name();
		if ( $last_name ) {
			$user_data['ln'] = Ratul_ACT_Hasher::hash_name( $last_name );
		}

		// Location fields
		$city = $order->get_billing_city();
		if ( $city ) {
			$user_data['ct'] = Ratul_ACT_Hasher::hash_city( $city );
		}
		$state = $order->get_billing_state();
		if ( $state ) {
			$user_data['st'] = Ratul_ACT_Hasher::hash_state( $state );
		}
		$zip = $order->get_billing_postcode();
		if ( $zip ) {
			$user_data['zp'] = Ratul_ACT_Hasher::hash_zip( $zip );
		}
		if ( $country ) {
			$user_data['country'] = Ratul_ACT_Hasher::hash_country( $country );
		}

		return [
			'event_name'  => 'Purchase',
			'event_time'  => $order->get_date_created() ? $order->get_date_created()->getTimestamp() : time(),
			'event_id'    => Ratul_ACT_Dedup::generate_event_id( 'offline_purchase_' . $order_id ),
			'value'       => $total,
			'currency'    => $currency,
			'order_id'    => $order_id,
			'platform'    => $platform,
	...
	private static function dispatch( array $events, string $platform ): void {
		$settings = get_option( 'ratul_act_settings', [] );
