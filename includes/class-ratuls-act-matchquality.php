...
/**
 * Ratuls_ACT_MatchQuality  v2.0
 *
 * Feature #3 — Real-Time Event Match Quality (EMQ) Scorer.
 *
 * HIGH FIX (H-2) in v2.0:
 *   get_daily_averages() previously read from ratuls_act_debug_log which is
 ...
 *
 *   Fixed: now reads from ratuls_act_event_stats — the always-on stats store
...
class Ratuls_ACT_MatchQuality {
     ...
     */
    public static function annotate( Ratuls_ACT_Event $event ): array {
     ...
     *
     * HIGH FIX (H-2 — v2.0):
     *   Now reads ratuls_act_event_stats (always-on) instead of
     *   ratuls_act_debug_log (debug_mode=1 only → always empty in prod).
     *
    ...
    public static function get_daily_averages( int $days = 7 ): array {
        ...
        $stats   = get_option( 'ratuls_act_event_stats', [] );
        $has_emq = false;
