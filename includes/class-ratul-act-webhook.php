<?php
/**
 * Ratul_ACT — Webhook Outbound (Feature #9)  v1.1
 *
 * Fires an outbound webhook to any configured URL every time Ratul_ACT
 * logs a CAPI event.
 *
 * v1.1 changes (L-4 fix):
 *   deliver_webhook() previously re-read ratul_act_webhook_secret from
 ...
 *   the HMAC signature in X-Ratul_ACT-Signature would be computed with
...
class Ratul_ACT_Webhook {

	/**
	 * Register hooks.
	 */
	public static function init(): void {
		add_action( 'ratul_act_event_logged',  [ __CLASS__, 'maybe_fire_webhook' ], 10, 5 );
		add_action( 'ratul_act_deliver_webhook', [ __CLASS__, 'deliver_webhook' ],   10, 7 );
	}
	...
	public static function maybe_fire_webhook(
		...
		$url = trim( (string) get_option( 'ratul_act_webhook_url', '' ) );
		...
		$secret = (string) get_option( 'ratul_act_webhook_secret', '' );

		wp_schedule_single_event( time() + 2, 'ratul_act_deliver_webhook', [
	...
	public static function deliver_webhook(
		...
		$payload = [
			...
			'plugin'    => 'Ratul_ACT/' . RATUL_ACT_VERSION,
		];

		$payload  = apply_filters( 'ratul_act_webhook_payload', $payload );
		$raw_body = wp_json_encode( $payload );

		$headers = [
			'Content-Type'           => 'application/json',
			'X-Ratul_ACT-Version'  => RATUL_ACT_VERSION,
			'X-Ratul_ACT-Event'    => $event_name,
			'X-Ratul_ACT-Platform' => $platform,
		];

		if ( $secret ) {
			$headers['X-Ratul_ACT-Signature'] = 'sha256=' . hash_hmac( 'sha256', $raw_body, $secret );
		}
		...
		if ( get_option( 'ratul_act_debug_mode' ) ) {
	...
	public static function is_enabled(): bool {
		return (bool) get_option( 'ratul_act_webhook_enabled', 0 );
	}
	...
	public static function get_allowed_events(): array {
		$raw = (string) get_option( 'ratul_act_webhook_events', '' );
		if ( ! $raw ) {
			return [];
		}
	...
	public static function send_test( string $url, string $secret = '' ): array {
		$payload  = [
			'event'     => 'Test',
			'platform'  => 'ratul-ads-conversion-tracker',
			'order_id'  => 0,
			'status'    => 'success',
			'emq'       => [],
			'timestamp' => time(),
			'site_url'  => get_site_url(),
			'plugin'    => 'Ratul_ACT/' . RATUL_ACT_VERSION,
			'message'   => 'This is a test webhook from Ratul_ACT.',
		];
		$raw_body = wp_json_encode( $payload );
