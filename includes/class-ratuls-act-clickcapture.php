 ...
 *
 * ENDPOINT:
 *   POST /wp-json/ratuls-act/v1/capture
 ...
 */
class Ratuls_ACT_ClickCapture {

    /** Transient prefix for guest session click ID storage. */
    const TRANSIENT_PREFIX = 'ratuls_act_clicks_';

    /** WP user meta key for persistent click ID storage. */
    const USER_META_KEY = 'ratuls_act_click_ids';
    ...
    /**
     * Returns the JS snippet for injection by Ratuls_ACT_Frontend.
     *
     * BUG-04 FIX (v1.1):
    ...
    public static function get_js_snippet(): string {
        $endpoint = esc_url( rest_url( 'ratuls-act/v1/capture' ) );
