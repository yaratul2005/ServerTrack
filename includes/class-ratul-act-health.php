 ...
 */
class Ratul_ACT_Health {

    const OPTION_KEY = 'ratul_act_health_status';

    public static function init(): void {
        add_action( 'ratul_act_daily_health_check', [ self::class, 'run_health_check' ] );

        if ( ! wp_next_scheduled( 'ratul_act_daily_health_check' ) ) {
            wp_schedule_event( time(), 'daily', 'ratul_act_daily_health_check' );
        }
    }
    ...
    private static function check_meta(): array {
        $pixel_id = get_option( 'ratul_act_meta_pixel_id', '' );
        $token = get_option( 'ratul_act_meta_access_token', '' );
    ...
    private static function check_tiktok(): array {
        $token = get_option( 'ratul_act_tiktok_access_token', '' );
        if ( empty( $token ) ) {
            return [ 'status' => 'warning', 'message' => 'Not configured' ];
        }
    ...
    private static function check_google(): array {
        ...
        $api_secret = get_option( 'ratul_act_google_api_secret', '' );
        if ( empty( $api_secret ) ) {
            return [ 'status' => 'warning', 'message' => 'Not configured' ];
        }
        return [ 'status' => 'ok', 'message' => 'Active' ];
    ...
    private static function send_alerts_if_needed( array $status ): void {
        ...
        $subject = '[Ratul_ACT] Action Required: CAPI Tracking Failed';
        $message = "Ratul_ACT detected authentication errors that are preventing events from being sent.\n\n" . implode( "\n", $errors );
