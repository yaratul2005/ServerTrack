 ...
 *
 *   - Every logged-in WP user gets a persistent, stable `ratul_act_uid`
 ...
 *
 * Usage:
 *   Ratul_ACT_Identity::init()    — call from Core::init()
 *   Ratul_ACT_Identity::get_external_id_for_order( $order )  — in WooCommerce source
 *   Ratul_ACT_Identity::get_external_id_for_user( $user_id ) — for logged-in events
 */
class Ratul_ACT_Identity {

    const USER_META_KEY = 'ratul_act_uid';
    ...
    public static function get_external_id_for_order( WC_Abstract_Order $order ): string {
        ...
        return Ratul_ACT_Hasher::hash( (string) $order->get_id() );
    }
    ...
    public static function get_external_id_for_user( int $user_id ): string {
        return Ratul_ACT_Hasher::hash( self::ensure_uid( $user_id ) );
    }
    ...
    public static function stitch_order_to_user( int $order_id, array $posted_data, WC_Order $order ): void {
        ...
        $order->update_meta_data( '_ratul_act_external_id', $hashed_uid );
