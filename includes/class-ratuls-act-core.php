...
/**
 * Ratuls_ACT_Core — backward-compatibility shim (v6.0.3+)
 *
 * This class is intentionally a no-op for its bootstrap. In v6.0.2 and earlier,
 * Ratuls_ACT_Core was a competing bootstrap system inside includes/ that was
 * never actually loaded or called from the main plugin file (ratuls-act.php).
 *
 * As of v6.0.3 the single authoritative bootstrap is ratuls_act_init() in
 * ratuls-act.php. Ratuls_ACT_Core::init() is kept here as a safe no-op.
 *
 * BUG-FIX (v6.0.5):
 *   dispatch_to_all() and dispatch_to_platforms() were missing entirely from
 *   this shim. Ratuls_ACT_Source_WooCommerce calls them on every WooCommerce
 ...
 *   (Ratuls_ACT_Meta, Ratuls_ACT_TikTok, Ratuls_ACT_Google) and
 *   Ratuls_ACT_Dedup for per-platform already-sent guards.
 */
class Ratuls_ACT_Core {

    /**
     * No-op. The real bootstrap runs via ratuls_act_init() at plugins_loaded.
     *
     * @since 6.0.3
     */
    public static function init(): void {
        _doing_it_wrong(
            __METHOD__,
            'Ratuls_ACT_Core::init() is a no-op shim since v6.0.3. ' .
            'The authoritative bootstrap is ratuls_act_init() in ratuls-act.php.',
            '6.0.3'
        );
    }
     ...
     *
     * This is the method called by Ratuls_ACT_Source_WooCommerce for every
     * WooCommerce CAPI event. It was missing from this shim, causing a PHP
     * Fatal Error on every hook firing.
     *
     * @param Ratuls_ACT_Event  $event     Fully populated event DTO.
     ...
     *
     * Checks Ratuls_ACT_Dedup::already_sent() per platform before sending,
     * then marks each successfully dispatched platform via
     * Ratuls_ACT_Dedup::mark_sent().
     *
     * @param Ratuls_ACT_Event  $event      Fully populated event DTO.
    ...
    public static function dispatch_to_platforms(
        ...
        Ratuls_ACT_Dispatcher::dispatch( $event, $platforms, $key );
    }
}
