 ...
 */
class Ratuls_ACT_Stream {

    const STREAM_OPTION = 'ratuls_act_live_stream_buffer';
    const MAX_EVENTS = 200;

    public static function init(): void {
        add_action( 'rest_api_init', [ self::class, 'register_routes' ] );
        // Hook into event logging to populate the stream buffer
        add_action( 'ratuls_act_event_dispatched', [ self::class, 'push_to_buffer' ], 10, 3 );
    }

    public static function register_routes(): void {
        // SSE Endpoint
        register_rest_route( 'ratuls-act/v1', '/stream', [
            'methods'             => 'GET',
        ...
        // Test Fire Endpoint
        register_rest_route( 'ratuls-act/v1', '/test-fire', [
            'methods'             => 'POST',
    ...
    public static function push_to_buffer( string $platform, Ratuls_ACT_Event $event, array $response ): void {
        $entry = [
            'time'       => time(),
            'platform'   => $platform,
    ...
    public static function handle_test_fire( WP_REST_Request $request ) {
        ...
        $event_id   = Ratuls_ACT_Dedup::generate_event_id( 'test_' . time() );

        $event = new Ratuls_ACT_Event( $event_name, $event_id );
        $event->set_user_data( [
            'email' => hash( 'sha256', 'test@example.com' ),
            'ip'    => Ratuls_ACT_Enrichment::get_client_ip(),
        ] );
