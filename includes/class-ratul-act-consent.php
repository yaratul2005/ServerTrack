 ...
 *
 * Storage:
 *   Order meta key: _ratul_act_consent
 *   Value: serialised array [ 'meta' => true/false, 'google' => true/false, 'tiktok' => true/false ]
 *   Written by: Ratul_ACT_Consent::capture_for_order( $order_id )
 *   Read by:    Ratul_ACT_Consent::is_granted( $platform, $order_id )
 */
class Ratul_ACT_Consent {

    /** Order meta key where per-order consent snapshot is stored. */
    const ORDER_META_KEY = '_ratul_act_consent';
    ...
    public static function is_granted( string $platform, ?int $order_id = null ): bool {
        $mode = get_option( 'ratul_act_consent_mode', 'none' );
        ...
        if ( self::is_cron_or_cli() ) {
            if ( 'manual' === $mode ) {
                return (bool) apply_filters( 'ratul_act_consent_granted', false, $platform );
            }
            ...
            Ratul_ACT_Logger::warning(
                ...
                . (int) ( $order_id ?? 0 )
                . '. Ensure Ratul_ACT_Consent::capture_for_order() is called at checkout. '
        ...
        if ( 'manual' === $mode ) {
            $granted = apply_filters( 'ratul_act_consent_granted', false, $platform );
            if ( ! has_filter( 'ratul_act_consent_granted' ) ) {
                if ( class_exists( 'Ratul_ACT_Logger' ) ) {
                    Ratul_ACT_Logger::warning(
                        'Manual consent mode active but ratul_act_consent_granted filter not defined. '
