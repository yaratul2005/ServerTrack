...
class Ratuls_ACT_Catalog {
    ...
    public static function init(): void {
        ...
        add_filter( 'ratuls_act_view_content_custom_data',  [ __CLASS__, 'enrich_view_content' ],  10, 2 );
        // Enrich AddToCart events (product ID passed as context)
        add_filter( 'ratuls_act_add_to_cart_custom_data',   [ __CLASS__, 'enrich_add_to_cart' ],   10, 2 );
        // Enrich Purchase events (already has order; we upgrade contents array)
        add_filter( 'ratuls_act_purchase_custom_data',      [ __CLASS__, 'enrich_purchase' ],      20, 2 );
    }
    ...
    public static function get_product_catalog_data( WC_Product $product ): array {
        ...
        if ( ! $brand ) {
            $brand = apply_filters( 'ratuls_act_default_brand', get_bloginfo( 'name' ), $product );
        }
        ...
        return apply_filters( 'ratuls_act_product_catalog_data', $data, $product );
    }
