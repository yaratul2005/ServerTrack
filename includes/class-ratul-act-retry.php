 ...
 *   ratul_act_dedup_{key} namespace (same system used by Subscriptions source).
 ...
 *      AJAX action (ratul_act_drain_retries) can call
 *      Ratul_ACT_Retry::process_queue() without knowing the internal
 ...
 */
class Ratul_ACT_Retry {

    /** Option key for the pending retry queue. */
    const QUEUE_OPTION = 'ratul_act_retry_queue';
    ...
    public static function init(): void {
        add_action( 'ratul_act_process_retry_queue', [ self::class, 'process' ] );
    }
    ...
    public static function process(): void {
        $lock_key = 'ratul_act_retry_processing_lock';
        ...
        foreach ( $queue as $uid => $item ) {
            ...
            if ( ( $result['status'] ?? '' ) === 'success' ) {
                ...
                if ( '' !== $dedup_key ) {
                    ...
                    Ratul_ACT_Dedup::mark_string_sent( $dedup_key, $platform );
                } elseif ( $order_id > 0 ) {
                    // Standard WooCommerce order: use the order-meta dedup API.
                    Ratul_ACT_Dedup::mark_as_sent( $order_id, $platform );
                }

                Ratul_ACT_Logger::info(
     ...
     *
     * Added in v2.2 so Ratul_ACT_Dashboard::ajax_drain_retries() can call
     * Ratul_ACT_Retry::process_queue() via the stable public API, while
    ...
    private static function dispatch_retry( string $platform, array $event_args ): array {
        try {
            ...
            switch ( $platform ) {
                case 'meta':
                    return Ratul_ACT_Meta::send( $event );
                case 'tiktok':
                    return Ratul_ACT_TikTok::send( $event );
                case 'google':
                    return Ratul_ACT_Google::send( $event );
                default:
    ...
    /**
     * Convert a Ratul_ACT_Event into a serialisable args array for the queue.
     *
     ...
     * @return array
     */
    public static function event_to_args( Ratul_ACT_Event $event ): array {
        $args = [
