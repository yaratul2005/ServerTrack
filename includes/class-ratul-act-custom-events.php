 ...
 * handled by ratul-ads-conversion-tracker-pixel.js via the REST bridge.
 */
class Ratul_ACT_CustomEvents {

    public static function init() {
        // PHP action hook — themes/plugins call this directly
        add_action( 'ratul_act_custom_event', [ self::class, 'handle_custom_event' ], 10, 2 );
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // PHP ACTION HOOK  —  do_action( 'ratul_act_custom_event', $name, $params )
    // ─────────────────────────────────────────────────────────────────────────
    ...
    public static function handle_custom_event( string $event_name, array $params = [] ) {
        ...
        $event_id = Ratul_ACT_Dedup::generate_event_id( $event_name . '_php_' . time() . '_' . wp_rand() );
    ...
    public static function on_search_query( WP_Query $query ) {
        ...
        $dedup_key = 'ratul_act_search_' . md5( $search_query . '_' . $session_id );
        if ( get_transient( $dedup_key ) ) return;
        set_transient( $dedup_key, 1, 5 * MINUTE_IN_SECONDS );

        $event_id = Ratul_ACT_Dedup::generate_event_id( 'search_' . md5( $search_query ) . '_' . time() );
        $event    = new Ratul_ACT_Event( 'Search', $event_id );
        $event->set_user_data( self::build_request_user_data() );
        ...
        if ( $meta_on && Ratul_ACT_Consent::is_granted( 'meta' ) ) {
            Ratul_ACT_Meta::send( $event );
        }
        if ( $tiktok_on && Ratul_ACT_Consent::is_granted( 'tiktok' ) ) {
            Ratul_ACT_TikTok::send( $event );
        }
    }
    ...
    public static function on_view_category() {
        ...
        if ( is_product_category() || is_product_tag() ) {
            ...
            $category_name = __( 'Shop', 'ratul-ads-conversion-tracker' );
        }

        // Dedup: once per category per session (10 min TTL)
        $session_id = function_exists( 'WC' ) && WC()->session
            ? (string) WC()->session->get_customer_id() : '';
        $dedup_key = 'ratul_act_vc_' . md5( $category_name . '_' . $session_id );
        if ( get_transient( $dedup_key ) ) return;
        set_transient( $dedup_key, 1, 10 * MINUTE_IN_SECONDS );

        $event_id = Ratul_ACT_Dedup::generate_event_id( 'viewcat_' . md5( $category_name ) . '_' . time() );
        $event    = new Ratul_ACT_Event( 'ViewCategory', $event_id );
        $event->set_user_data( self::build_request_user_data() );
        $event->set_custom_data( [
    ...
    // WooCommerce registration is handled by Ratul_ACT_WooCommerce::on_new_customer
    // ─────────────────────────────────────────────────────────────────────────
    ...
    public static function on_user_register( int $user_id ) {
        ...
        $event_id = Ratul_ACT_Dedup::generate_event_id( 'reg_wp_' . $user_id );
        $event    = new Ratul_ACT_Event( 'CompleteRegistration', $event_id );

        $user_data = self::build_request_user_data();
        if ( $user->user_email ) {
            $user_data['email'] = Ratul_ACT_Hasher::hash_email( $user->user_email );
        }
        if ( $user->first_name ) {
            $user_data['first_name'] = Ratul_ACT_Hasher::hash_name( $user->first_name );
        }
        if ( $user->last_name ) {
            $user_data['last_name'] = Ratul_ACT_Hasher::hash_name( $user->last_name );
        }
