...
/**
 * Ratuls_ACT_Dispatcher  v1.0
 *
 * Async dispatch pipeline for Ratuls_ACT.
 * Uses wp_remote_post loopback mechanism to avoid blocking checkout.
 */
class Ratuls_ACT_Dispatcher {

    const ACTION_NAME = 'ratuls_act_async_dispatch';
     ...
     */
    public static function dispatch( Ratuls_ACT_Event $event, array $platforms, $dedup_key = null ): void {
        $key = (string) ( $dedup_key ?? $event->event_id );
        $secret = defined( 'SECURE_AUTH_KEY' ) ? SECURE_AUTH_KEY : 'ratuls_act_salt';
        $token  = hash_hmac( 'sha256', self::ACTION_NAME, $secret );
        $payload = [
            'action'    => self::ACTION_NAME,
            'event'     => wp_json_encode( Ratuls_ACT_Retry::event_to_args( $event ) ),
    ...
    public static function process_dispatch(): void {
        ...
        $event = new Ratuls_ACT_Event( $event_data['event_name'] ?? '', $event_data['event_id'] ?? '' );
        ...
        foreach ( $platforms as $platform ) {
            ...
            switch ( $platform ) {
                ...
                case 'google':
                    ...
                    break;
                case 'snapchat':
                    if ( get_option( 'ratuls_act_snapchat_enabled', 0 ) ) {
                        $result = Ratuls_ACT_Snapchat::send( $event );
                    }
                    break;
                case 'pinterest':
                    if ( get_option( 'ratuls_act_pinterest_enabled', 0 ) ) {
                        $result = Ratuls_ACT_Pinterest::send( $event );
                    }
                    break;
                case 'linkedin':
                    if ( get_option( 'ratuls_act_linkedin_enabled', 0 ) ) {
                        $result = Ratuls_ACT_LinkedIn::send( $event );
                    }
                    break;
            }

            if ( isset( $result['status'] ) && $result['status'] === 'success' ) {
                Ratuls_ACT_Dedup::mark_sent( $dedup_key, $platform );
            } elseif ( isset( $result['status'] ) && $result['status'] === 'error' ) {
                Ratuls_ACT_Retry::maybe_queue( $platform, $result, $event_data );
            }
        }
    }
}
