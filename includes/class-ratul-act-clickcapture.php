 ...
 *
 * ENDPOINT:
 *   POST /wp-json/ratul-ads-conversion-tracker/v1/capture
 ...
 */
class Ratul_ACT_ClickCapture {

    /** Transient prefix for guest session click ID storage. */
    const TRANSIENT_PREFIX = 'ratul_act_clicks_';

    /** WP user meta key for persistent click ID storage. */
    const USER_META_KEY = 'ratul_act_click_ids';
    ...
    public static function register_endpoint(): void {
        register_rest_route(
            'ratul-ads-conversion-tracker/v1',
            '/capture',
            [
                'methods'             => 'POST',
    ...
    /**
     * Returns the JS snippet for injection by Ratul_ACT_Frontend.
     *
     * BUG-04 FIX (v1.1):
    ...
    public static function get_js_snippet(): string {
        $endpoint = esc_url( rest_url( 'ratul-ads-conversion-tracker/v1/capture' ) );
