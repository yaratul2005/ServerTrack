<?php
/**
 * Ratuls_ACT — Browser Pixel event_id Injection (Feature #5)
 *
 ...
 *      it in order meta: _ratuls_act_event_id_{event_name}
 ...
 *   Now requires a valid nonce (ratuls_act_event_id) for browser requests,
...
class Ratuls_ACT_PixelDedup {
	...
	public static function init(): void {
		...
		// Commented out to prevent timing issue (InitiateCheckout fired before fbq was initialized). Handled globally via ratuls-act-pixel.js.
		...
		// Commented out to prevent duplicate browser Purchase fires since ratuls-act-pixel.js already handles this using the enqueued config.
	...
	public static function store_purchase_event_id( WC_Order $order ): void {
		...
		$order->update_meta_data( '_ratuls_act_event_id_purchase', $event_id );
		$order->update_meta_data( '_ratuls_act_event_id', $event_id );
		$order->save();
	}
	...
	public static function get_order_event_id( int $order_id, string $event_name = 'purchase' ): string {
		...
		return (string) $order->get_meta( '_ratuls_act_event_id_' . $event_name ) ?: '';
	}
	...
	public static function inject_purchase_dedup_snippet( int $order_id ): void {
		...
		echo "<script>\n";
		echo "/* Ratuls_ACT — Purchase pixel dedup (event_id matches CAPI) */\n";
	...
	public static function inject_initiate_checkout_id(): void {
		...
		// phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped
		echo "<script>\n";
		echo "/* Ratuls_ACT — InitiateCheckout pixel dedup */\n";
		echo "if(typeof fbq==='function'){";
	...
	public static function inject_add_to_cart_data(): void {
		...
		echo '<input type="hidden" id="ratuls-act-atc-event-id" value="' . esc_attr( $event_id ) . '">' . "\n";
	}

	// ─────────────────────────────────────────────────────────────────────────
	// REST endpoint: GET /wp-json/ratuls-act/v1/event-id
	// ─────────────────────────────────────────────────────────────────────────

	public static function register_rest_endpoint(): void {
		register_rest_route( 'ratuls-act/v1', '/event-id', [
			 ...
			 *   use). Browser callers must supply a valid 'ratuls_act_event_id' nonce
	 ...
	 * 'ratuls_act_event_id' action so unauthenticated enumeration
	...
	public static function rest_permission_check( WP_REST_Request $request ) {
		...
		if ( $nonce && wp_verify_nonce( $nonce, 'ratuls_act_event_id' ) ) {
			return true;
		}
