 ...
 *
 * Storage:
 *   Order meta key: _ratuls_act_consent
 *   Value: serialised array [ 'meta' => true/false, 'google' => true/false, 'tiktok' => true/false ]
 *   Written by: Ratuls_ACT_Consent::capture_for_order( $order_id )
 *   Read by:    Ratuls_ACT_Consent::is_granted( $platform, $order_id )
 */
class Ratuls_ACT_Consent {

    /** Order meta key where per-order consent snapshot is stored. */
    const ORDER_META_KEY = '_ratuls_act_consent';
    ...
    public static function is_granted( string $platform, ?int $order_id = null ): bool {
        $mode = get_option( 'ratuls_act_consent_mode', 'none' );
        ...
        if ( self::is_cron_or_cli() ) {
            if ( 'manual' === $mode ) {
                return (bool) apply_filters( 'ratuls_act_consent_granted', false, $platform );
            }
            ...
            Ratuls_ACT_Logger::warning(
                ...
                . (int) ( $order_id ?? 0 )
                . '. Ensure Ratuls_ACT_Consent::capture_for_order() is called at checkout. '
        ...
        if ( 'manual' === $mode ) {
            $granted = apply_filters( 'ratuls_act_consent_granted', false, $platform );
            if ( ! has_filter( 'ratuls_act_consent_granted' ) ) {
                if ( class_exists( 'Ratuls_ACT_Logger' ) ) {
                    Ratuls_ACT_Logger::warning(
                        'Manual consent mode active but ratuls_act_consent_granted filter not defined. '
