<?php
/**
 * Plugin Name:       Ratul Ads Conversion Tracker
 * Plugin URI:        https://github.com/yaratul2005/Ratul_ACT
 ...
 * Text Domain:       ratul-ads-conversion-tracker
 */
 ...
 * ----------------------
 *   BUG-FIX-1: Ratul_ACT_Hasher::event_id() was missing — fatal error on
 *              every WooCommerce CAPI event from the v3.x source layer.
 *   BUG-FIX-2: Ratul_ACT_Source_WooCommerce::init() was never called —
 *              entire v3.x extended WooCommerce source was dead code.
 *   BUG-FIX-3: Cart abandonment option key mismatch fixed.
 *              (ratul_act_source_abandonment_enabled vs
 *               ratul_act_source_cart_abandonment_enabled)
 ...
 *
 *   1. The ORIGINAL flat system in this file (ratul_act_init) loaded only
 *      ~15 classes and missed: Ratul_ACT_Frontend, Ratul_ACT_CustomEvents,
 *      Ratul_ACT_Retry (call), and all v3.x WooCommerce source classes.
 *
 *   2. The NEWER Ratul_ACT_Core::init() system in
 *      includes/class-ratul-ads-conversion-tracker-core.php was never require_once'd or called
 ...
 * skipped.
 *
 * Fix: one authoritative ratul_act_load_classes() + ratul_act_init() here.
 * class-ratul-ads-conversion-tracker-core.php is kept as a backward-compat shim (no-op).
 */

define( 'RATUL_ACT_VERSION', '6.0.4' );
define( 'RATUL_ACT_DIR',     plugin_dir_path( __FILE__ ) );
define( 'RATUL_ACT_URL',     plugin_dir_url( __FILE__ ) );
...
// ─────────────────────────────────────────────────────────────────────────────
function ratul_act_init(): void {
    ratul_act_load_classes();
    ratul_act_run_upgrade();

    // ── Core infrastructure ───────────────────────────────────────────────────
    Ratul_ACT_Dispatcher::init();
    Ratul_ACT_CookieHelper::init();
    Ratul_ACT_Proxy::init();
    Ratul_ACT_Identity::init();
    Ratul_ACT_ClickCapture::init();
    Ratul_ACT_OfflineConversion::init();
    Ratul_ACT_PixelDedup::init();
    Ratul_ACT_DedupEngine::init();
    Ratul_ACT_Enrichment::init();
    Ratul_ACT_Health::init();
    Ratul_ACT_Stream::init();
    Ratul_ACT_Attribution::init();
    Ratul_ACT_License::init();
    Ratul_ACT_LTV::init();
    Ratul_ACT_Catalog::init();
    Ratul_ACT_Webhook::init();
    // BUG-3 FIX: Retry::init() was never called → queue never processed.
    Ratul_ACT_Retry::init();
    // BUG-2 FIX: CustomEvents::init() was never called.
    Ratul_ACT_CustomEvents::init();

    // ── WooCommerce sources ───────────────────────────────────────────────────
    if ( class_exists( 'WooCommerce' ) ) {
        Ratul_ACT_WooCommerce::init();
        ...
        Ratul_ACT_Source_WooCommerce::init();

        // Renewals (WooCommerce Subscriptions plugin).
        if ( class_exists( 'WC_Subscriptions' ) ) {
            Ratul_ACT_WooRenewals::init();
            Ratul_ACT_Subscriptions::init();
        }

        // BUG-FIX-3: was checking 'ratul_act_source_abandonment_enabled'
        // but admin saves to 'ratul_act_source_cart_abandonment_enabled'.
        if ( get_option( 'ratul_act_source_cart_abandonment_enabled', 0 ) ) {
            Ratul_ACT_CartAbandonment::init();
            Ratul_ACT_WooAbandonment::init();
        }

        // Order lifecycle status events (on-hold, failed, cancelled).
        if ( get_option( 'ratul_act_source_order_status_enabled', 1 ) ) {
            Ratul_ACT_WooOrderStatus::init();
        }

        // AddToWishlist events — opt-in.
        if ( get_option( 'ratul_act_source_wishlist_enabled', 0 ) ) {
            Ratul_ACT_WooWishlist::init();
        }

        // Partial refund events.
        if ( get_option( 'ratul_act_source_partial_refund_enabled', 1 ) ) {
            Ratul_ACT_WooPartialRefund::init();
        }
    }

    // ── Optional third-party sources ─────────────────────────────────────────
    if ( class_exists( 'WPCF7' ) && get_option( 'ratul_act_source_cf7_enabled', 0 ) ) {
        Ratul_ACT_CF7::init();
    }
    if ( class_exists( 'Easy_Digital_Downloads' ) && get_option( 'ratul_act_source_edd_enabled', 0 ) ) {
        Ratul_ACT_EDD::init();
    }

    // ── Admin ─────────────────────────────────────────────────────────────────
    if ( is_admin() ) {
        Ratul_ACT_Dashboard::init();
        Ratul_ACT_Admin::init();
    }

    // ── Frontend pixel ────────────────────────────────────────────────────────
    if ( ! is_admin() ) {
        Ratul_ACT_Frontend::init();
    }
}
add_action( 'plugins_loaded', 'ratul_act_init', 20 );

// ─────────────────────────────────────────────────────────────────────────────
// Upgrade guard — version-keyed so it only runs once per version bump.
// ─────────────────────────────────────────────────────────────────────────────
function ratul_act_run_upgrade(): void {
    $installed = get_option( 'ratul_act_db_version', '0' );
    if ( version_compare( $installed, RATUL_ACT_VERSION, '>=' ) ) {
        return; // Nothing to do.
    }
    ratul_act_create_tables();
    ratul_act_register_defaults();
    update_option( 'ratul_act_db_version', RATUL_ACT_VERSION );
}

function ratul_act_create_tables(): void {
    global $wpdb;
...
function ratul_act_register_defaults(): void {
    $defaults = [
        // Core toggles
        'ratul_act_enabled'                        => 1,
        'ratul_act_debug_mode'                     => 0,
        'ratul_act_debug_log'                      => [],
        'ratul_act_retry_queue'                    => [],
        // Consent
        'ratul_act_consent_mode'                   => 'none',
        // Platform toggles
        'ratul_act_meta_enabled'                   => 0,
        'ratul_act_meta_pixel_id'                  => '',
        'ratul_act_meta_access_token'              => '',
        'ratul_act_meta_test_event_code'           => '',
        'ratul_act_google_enabled'                 => 0,
        'ratul_act_google_conversion_id'           => '',
        'ratul_act_google_conversion_label'        => '',
        'ratul_act_google_refresh_token'           => '',
        'ratul_act_google_client_id'               => '',
        'ratul_act_google_client_secret'           => '',
        'ratul_act_tiktok_enabled'                 => 0,
        'ratul_act_tiktok_pixel_id'                => '',
        'ratul_act_tiktok_access_token'            => '',
        // Source toggles
        'ratul_act_source_woo_enabled'                      => 1,
        'ratul_act_source_cart_abandonment_enabled'         => 0,
        'ratul_act_abandonment_window_minutes'              => 60,
        'ratul_act_source_order_status_enabled'             => 1,
        'ratul_act_source_wishlist_enabled'                 => 0,
        'ratul_act_source_partial_refund_enabled'           => 1,
        'ratul_act_source_cf7_enabled'                      => 0,
        'ratul_act_source_edd_enabled'                      => 0,
        'ratul_act_source_subscriptions_enabled'            => 0,
    ];
...
add_filter( 'cron_schedules', function ( array $schedules ): array {
    if ( ! isset( $schedules['every_five_minutes'] ) ) {
        $schedules['every_five_minutes'] = [
            'interval' => 300,
            'display'  => __( 'Every 5 Minutes', 'ratul-ads-conversion-tracker' ),
        ];
    }
    return $schedules;
} );
...
// ─────────────────────────────────────────────────────────────────────────────
add_action( 'ratul_act_cleanup_dedup', function() {
    global $wpdb;
    $wpdb->query( "DELETE FROM {$wpdb->prefix}ratul_act_dedup WHERE expires_at < NOW()" );
});
...
register_activation_hook( __FILE__, function (): void {
    ...
    if ( ! wp_next_scheduled( 'ratul_act_cleanup_dedup' ) ) {
        wp_schedule_event( time(), 'daily', 'ratul_act_cleanup_dedup' );
    }

    if ( ! wp_next_scheduled( 'ratul_act_process_retry_queue' ) ) {
        wp_schedule_event( time(), 'every_five_minutes', 'ratul_act_process_retry_queue' );
    }
    if ( get_option( 'ratul_act_source_cart_abandonment_enabled', 0 ) ) {
        if ( ! wp_next_scheduled( 'ratul_act_check_abandonment' ) ) {
            wp_schedule_event( time(), 'every_five_minutes', 'ratul_act_check_abandonment' );
        }
    }
} );

register_deactivation_hook( __FILE__, function (): void {
    wp_clear_scheduled_hook( 'ratul_act_process_retry_queue' );
    wp_clear_scheduled_hook( 'ratul_act_check_abandonment' );
} );
