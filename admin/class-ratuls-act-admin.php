 ...
 *
 *   C2  — ratuls_act_source_woo_enabled was in the Sources view but
 *         never registered. Added to ratuls_act_sources_settings.
 *
 *   C3  — Sources view used ratuls_act_source_abandonment_enabled;
 *         register_settings() had _cart_abandonment_enabled. Aligned
 *         both to ratuls_act_source_cart_abandonment_enabled.
 *
 *   C4  — ratuls_act_abandonment_window_minutes used in view but never
 *         registered. Added (integer, absint, default 60).
 *
 *   C5  — ratuls_act_source_cf7_enabled not registered. Added.
 *
 *   C6  — ratuls_act_source_edd_enabled not registered. Added.
 *
 *   C7  — ratuls_act_source_subscriptions_enabled registered but had
 *         no UI field. Added a Subscriptions row to the Sources view.
 *
 *   C8  — render_health_notice() checked for screen ID
 *         ratuls_act_page_ratuls-act-sources (non-existent). Removed.
 *
 ...
 *
 * Changes in v2.9:
 *   FIX A3 — Removed duplicate wp_ajax_ratuls_act_clear_log registration.
 *   FIX A4 — render_health_notice() now only renders on Ratuls_ACT admin pages.
 *
 * Changes in v2.8:
 *   FIX BUG-FIX-4 — register_settings() now registers the three source
 *   options that were previously missing.
 */
class Ratuls_ACT_Admin {
     ...
     */
    const TAB_GROUPS = [
        'general' => 'ratuls_act_general_settings',
        'meta'    => 'ratuls_act_meta_settings',
        'google'  => 'ratuls_act_google_settings',
        'tiktok'  => 'ratuls_act_tiktok_settings',
        'sources' => 'ratuls_act_sources_settings',
        'license' => 'ratuls_act_license_settings',
    ];

    private static function settings_url( string $tab = '', array $extra = [] ): string {
        $args = array_merge( [ 'page' => 'ratuls-act-settings' ], $extra );
        if ( $tab !== '' ) {
            $args['tab'] = $tab;
        }
    ...
    public static function init() {
        ...
        add_action( 'admin_action_ratuls_act_manual_purchase', [ self::class, 'handle_manual_purchase_action' ] );
        ...
        add_action( 'wp_ajax_ratuls_act_test_event',          [ self::class, 'ajax_test_event' ] );
        add_action( 'wp_ajax_ratuls_act_get_logs',            [ self::class, 'ajax_get_logs' ] );
        add_action( 'wp_ajax_ratuls_act_get_dashboard_stats', [ self::class, 'ajax_get_dashboard_stats' ] );
        ...
        add_action( 'admin_action_ratuls_act_mark_fraud', [ self::class, 'handle_mark_fraud_action' ] );
    }
    ...
    public static function enqueue_assets( string $hook ) {
        $allowed_hooks = [
            'settings_page_ratuls-act',
            'ratuls_act_page_ratuls-act-settings',
            'toplevel_page_ratuls-act',
        ];
        if ( ! in_array( $hook, $allowed_hooks, true ) ) return;

        wp_enqueue_style(
            'ratuls-act-admin',
            RATULS_ACT_URL . 'admin/assets/admin.css',
            [],
            RATULS_ACT_VERSION
        );
        wp_enqueue_script(
            'ratuls-act-admin',
            RATULS_ACT_URL . 'admin/assets/admin.js',
            [ 'jquery' ],
            RATULS_ACT_VERSION,
            true
        );
        wp_localize_script( 'ratuls-act-admin', 'ratuls_act_admin', [
            'ajax_url'        => admin_url( 'admin-ajax.php' ),
            'nonce'           => wp_create_nonce( 'ratuls_act_admin_nonce' ),
            'dashboard_nonce' => wp_create_nonce( 'ratuls_act_dashboard' ),
            'platforms' => [
                'meta'   => [
                    'enabled'    => (bool) get_option( 'ratuls_act_meta_enabled', 0 ),
                    'configured' => (bool) (
                        get_option( 'ratuls_act_meta_pixel_id', '' ) &&
                        get_option( 'ratuls_act_meta_access_token', '' )
                    ),
                ],
                'google' => [
                    'enabled'    => (bool) get_option( 'ratuls_act_google_enabled', 0 ),
                    'configured' => (bool) get_option( 'ratuls_act_google_refresh_token', '' ),
                ],
                'tiktok' => [
                    'enabled'    => (bool) get_option( 'ratuls_act_tiktok_enabled', 0 ),
                    'configured' => (bool) (
                        get_option( 'ratuls_act_tiktok_pixel_id', '' ) &&
                        get_option( 'ratuls_act_tiktok_access_token', '' )
                    ),
                ],
            ],
        ] );
    ...
    public static function register_settings() {
        ...
        $general_options = [
            'ratuls_act_enabled'      => [ 'type' => 'integer', 'sanitize' => 'absint',                                  'default' => 1      ],
            'ratuls_act_test_mode'    => [ 'type' => 'integer', 'sanitize' => 'absint',                                  'default' => 0      ],
            'ratuls_act_consent_mode' => [ 'type' => 'string',  'sanitize' => [ self::class, 'sanitize_consent_mode' ],  'default' => 'none' ],
        ];
        self::register_group( 'ratuls_act_general_settings', $general_options );

        $meta_options = [
            'ratuls_act_meta_enabled'         => [ 'type' => 'integer', 'sanitize' => 'absint',              'default' => 0  ],
            'ratuls_act_meta_pixel_id'        => [ 'type' => 'string',  'sanitize' => 'sanitize_text_field', 'default' => '' ],
            'ratuls_act_meta_access_token'    => [ 'type' => 'string',  'sanitize' => 'sanitize_text_field', 'default' => '' ],
            'ratuls_act_meta_test_event_code' => [ 'type' => 'string',  'sanitize' => 'sanitize_text_field', 'default' => '' ],
            'ratuls_act_meta_am_email'   => [ 'type' => 'integer', 'sanitize' => 'absint', 'default' => 1 ],
            'ratuls_act_meta_am_phone'   => [ 'type' => 'integer', 'sanitize' => 'absint', 'default' => 1 ],
            'ratuls_act_meta_am_name'    => [ 'type' => 'integer', 'sanitize' => 'absint', 'default' => 1 ],
            'ratuls_act_meta_am_city'    => [ 'type' => 'integer', 'sanitize' => 'absint', 'default' => 1 ],
            'ratuls_act_meta_am_state'   => [ 'type' => 'integer', 'sanitize' => 'absint', 'default' => 1 ],
            'ratuls_act_meta_am_zip'     => [ 'type' => 'integer', 'sanitize' => 'absint', 'default' => 1 ],
            'ratuls_act_meta_am_country' => [ 'type' => 'integer', 'sanitize' => 'absint', 'default' => 1 ],
        ];
        self::register_group( 'ratuls_act_meta_settings', $meta_options );

        $google_options = [
            'ratuls_act_google_enabled'          => [ 'type' => 'integer', 'sanitize' => 'absint',              'default' => 0  ],
            'ratuls_act_google_customer_id'      => [ 'type' => 'string',  'sanitize' => 'sanitize_text_field', 'default' => '' ],
            'ratuls_act_google_conversion_id'    => [ 'type' => 'string',  'sanitize' => 'sanitize_text_field', 'default' => '' ],
            'ratuls_act_google_conversion_label' => [ 'type' => 'string',  'sanitize' => 'sanitize_text_field', 'default' => '' ],
            'ratuls_act_google_developer_token'  => [ 'type' => 'string',  'sanitize' => 'sanitize_text_field', 'default' => '' ],
            'ratuls_act_google_consent_ad_user_data' => [ 'type' => 'integer', 'sanitize' => 'absint', 'default' => 1 ],
            'ratuls_act_google_consent_ad_personalization' => [ 'type' => 'integer', 'sanitize' => 'absint', 'default' => 1 ],
            'ratuls_act_google_refresh_token'    => [ 'type' => 'string',  'sanitize' => 'sanitize_text_field', 'default' => '' ],
            'ratuls_act_google_client_id'        => [ 'type' => 'string',  'sanitize' => 'sanitize_text_field', 'default' => '' ],
            'ratuls_act_google_client_secret'    => [ 'type' => 'string',  'sanitize' => 'sanitize_text_field', 'default' => '' ],
        ];
        self::register_group( 'ratuls_act_google_settings', $google_options );

        $tiktok_options = [
            'ratuls_act_tiktok_enabled'      => [ 'type' => 'integer', 'sanitize' => 'absint',              'default' => 0  ],
            'ratuls_act_tiktok_pixel_id'     => [ 'type' => 'string',  'sanitize' => 'sanitize_text_field', 'default' => '' ],
            'ratuls_act_tiktok_access_token' => [ 'type' => 'string',  'sanitize' => 'sanitize_text_field', 'default' => '' ],
        ];
        self::register_group( 'ratuls_act_tiktok_settings', $tiktok_options );

        /*
         * C2  — ratuls_act_source_woo_enabled added (was in view, not registered).
         * C3  — key aligned to ratuls_act_source_cart_abandonment_enabled
         *       (view had _abandonment_enabled, a different name).
         * C4  — ratuls_act_abandonment_window_minutes added.
         * C5  — ratuls_act_source_cf7_enabled added.
         * C6  — ratuls_act_source_edd_enabled added.
         * C7  — ratuls_act_source_subscriptions_enabled was already here;
         *       a UI toggle has been added to the Sources view.
         */
        $license_options = [
            'ratuls_act_license_key' => [ 'type' => 'string', 'sanitize' => 'sanitize_text_field', 'default' => '' ],
        ];
        self::register_group( 'ratuls_act_license_settings', $license_options );

        $sources_options = [
            'ratuls_act_source_woo_enabled'              => [ 'type' => 'integer', 'sanitize' => 'absint', 'default' => 1  ],
            'ratuls_act_source_cart_abandonment_enabled' => [ 'type' => 'integer', 'sanitize' => 'absint', 'default' => 0  ],
            'ratuls_act_abandonment_window_minutes'      => [ 'type' => 'integer', 'sanitize' => 'absint', 'default' => 60 ],
            'ratuls_act_manual_purchase_enabled'         => [ 'type' => 'integer', 'sanitize' => 'absint', 'default' => 0  ],
            'ratuls_act_source_order_status_enabled'     => [ 'type' => 'integer', 'sanitize' => 'absint', 'default' => 1  ],
            'ratuls_act_source_wishlist_enabled'         => [ 'type' => 'integer', 'sanitize' => 'absint', 'default' => 0  ],
            'ratuls_act_source_partial_refund_enabled'   => [ 'type' => 'integer', 'sanitize' => 'absint', 'default' => 1  ],
            'ratuls_act_source_cf7_enabled'              => [ 'type' => 'integer', 'sanitize' => 'absint', 'default' => 0  ],
            'ratuls_act_source_edd_enabled'              => [ 'type' => 'integer', 'sanitize' => 'absint', 'default' => 0  ],
            'ratuls_act_source_subscriptions_enabled'    => [ 'type' => 'integer', 'sanitize' => 'absint', 'default' => 0  ],
        ];
        self::register_group( 'ratuls_act_sources_settings', $sources_options );
    }
    ...
    public static function handle_oauth_callback(): void {
        ...
        if ( empty( $_GET['ratuls_act_oauth'] ) || empty( $_GET['code'] ) ) {
            return;
        }
        if ( ! current_user_can( 'manage_options' ) ) {
            return;
        }
        check_admin_referer( 'ratuls_act_oauth_google', 'state' );
        $code = sanitize_text_field( wp_unslash( $_GET['code'] ) );
        if ( class_exists( 'Ratuls_ACT_Google_OAuth' ) ) {
            $result = Ratuls_ACT_Google_OAuth::exchange_code( $code );
            $tab    = 'google';
        ...
        if ( $order->get_meta( '_ratuls_act_manual_purchase_sent' ) === 'yes' ) {
            return $actions;
        }

        $url = wp_nonce_url( admin_url( 'admin.php?action=ratuls_act_manual_purchase&order_id=' . $order->get_id() ), 'ratuls_act_manual_purchase_' . $order->get_id() );
    ...
    public static function render_manual_purchase_meta_box( $post_or_order_object ) {
        ...
        $sent = $order->get_meta( '_ratuls_act_manual_purchase_sent' ) === 'yes' || in_array( 'meta', $sent_platforms, true );
    ...
    public static function handle_mark_fraud_action() {
        ...
        check_admin_referer( 'ratuls_act_mark_fraud_' . $order_id );

        $order = wc_get_order( $order_id );
        if ( $order ) {
            $order->update_meta_data( '_ratuls_act_manual_purchase_fraud', 'yes' );
            $order->save();
        }
    ...
    public static function handle_manual_purchase_action() {
        ...
        $order_id = isset( $_GET['order_id'] ) ? absint( $_GET['order_id'] ) : 0;
        check_admin_referer( 'ratuls_act_manual_purchase_' . $order_id );

        if ( class_exists( 'Ratuls_ACT_Source_WooCommerce' ) ) {
            $result = Ratuls_ACT_Source_WooCommerce::fire_manual_purchase( $order_id );
    ...
    public static function handle_license_actions(): void {
        ...
        if ( isset( $_POST['st_license_action'] ) && isset( $_POST['ratuls_act_license_key'] ) ) {
            check_admin_referer( 'ratuls_act_license_action', 'ratuls_act_license_nonce' );
            $action = sanitize_text_field( wp_unslash( $_POST['st_license_action'] ) );
            $key = sanitize_text_field( wp_unslash( $_POST['ratuls_act_license_key'] ) );

            if ( 'activate' === $action ) {
                $result = Ratuls_ACT_License::activate( $key );
                add_settings_error( 'ratuls_act_license_messages', 'st_license', $result['message'], $result['success'] ? 'success' : 'error' );
            } elseif ( 'deactivate' === $action ) {
                $result = Ratuls_ACT_License::deactivate();
                add_settings_error( 'ratuls_act_license_messages', 'st_license', $result['message'], $result['success'] ? 'success' : 'error' );
            }
        }
    }
    ...
    public static function handle_oauth_revoke(): void {
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended
        if ( empty( $_GET['ratuls_act_revoke'] ) ) {
            return;
        }
        if ( ! current_user_can( 'manage_options' ) ) {
            return;
        }
        check_admin_referer( 'ratuls_act_revoke_google' );
        if ( class_exists( 'Ratuls_ACT_Google_OAuth' ) ) {
            Ratuls_ACT_Google_OAuth::revoke();
        }
    ...
    // ─────────────────────────────────────────────────────────────────
    // Health Notice
    // C8 — Removed non-existent screen ID ratuls_act_page_ratuls-act-sources.
    //      The Settings page screen ID is ratuls_act_page_ratuls-act-settings.
    // ─────────────────────────────────────────────────────────────────
    ...
    public static function render_health_notice(): void {
        ...
        if ( get_option( 'ratuls_act_meta_enabled', 0 ) ) {
            if ( ! get_option( 'ratuls_act_meta_pixel_id', '' ) || ! get_option( 'ratuls_act_meta_access_token', '' ) ) {
                $issues[] = sprintf(
        ...
        if ( get_option( 'ratuls_act_google_enabled', 0 ) ) {
            if ( ! get_option( 'ratuls_act_google_refresh_token', '' ) ) {
                $issues[] = sprintf(
        ...
        if ( get_option( 'ratuls_act_tiktok_enabled', 0 ) ) {
            if ( ! get_option( 'ratuls_act_tiktok_pixel_id', '' ) || ! get_option( 'ratuls_act_tiktok_access_token', '' ) ) {
                $issues[] = sprintf(
    ...
    public static function render_page_header(): void {
        ...
        <div class="st-page-header">
            <div class="st-page-header-left">
                <img
                    src="<?php echo esc_url( RATULS_ACT_URL . 'admin/assets/bglogo.png' ); ?>"
                    alt="Ratuls-ACT"
                    width="46"
                    height="46"
                    class="st-logo-img"
            ...
            <div class="st-header-badges">
                <span class="st-header-version"><?php echo esc_html( 'v' . RATULS_ACT_VERSION ); ?></span>
                <nav>
    ...
    public static function render_page(): void {
        ...
        ?>
        <div class="wrap" id="ratuls-act-wrap">
    ...
    public static function ajax_test_event(): void {
        check_ajax_referer( 'ratuls_act_admin_nonce', 'nonce' );
        ...
        $event_id = Ratuls_ACT_Dedup::generate_event_id();
        $event    = ( new Ratuls_ACT_Event( $event_name, $event_id ) )
            ->set_custom_data( [ 'value' => 1.00, 'currency' => 'USD' ] );

        $result = [];
        if ( 'meta' === $platform && class_exists( 'Ratuls_ACT_Meta' ) ) {
            $result = Ratuls_ACT_Meta::send( $event );
        } elseif ( 'google' === $platform && class_exists( 'Ratuls_ACT_Google' ) ) {
            $result = Ratuls_ACT_Google::send( $event );
        } elseif ( 'tiktok' === $platform && class_exists( 'Ratuls_ACT_TikTok' ) ) {
            $result = Ratuls_ACT_TikTok::send( $event );
        }
    ...
    public static function ajax_get_logs(): void {
        ...
        $logs   = get_option( 'ratuls_act_debug_log', [] );
        $recent = array_slice( array_reverse( $logs ), 0, 200 );
        $count  = count( $logs );

        ob_start();
        Ratuls_ACT_Dashboard::render_log_rows( $recent );
        $html = ob_get_clean();
    ...
    public static function ajax_get_dashboard_stats(): void {
        ...
        $stats = get_option( 'ratuls_act_stats', [] );
