...
<form method="post" action="options.php">
    ...
    <p class="description"><?php esc_html_e( 'Choose which integrations fire server-side CAPI events. Each source covers both browser (pixel) and server (API) event delivery.', 'servertrack' ); ?></p>
    ...
    <table class="form-table" role="presentation">
            ...
            <td>
                ...
                <?php if ( ! $woo_active ) : ?>
                    <p class="description" style="color:#b32d2e;">
                        <?php esc_html_e( 'WooCommerce is not active.', 'servertrack' ); ?>
                    </p>
                <?php else : ?>
                    <p class="description">
                        <?php esc_html_e( 'Fires: ViewContent, AddToCart, InitiateCheckout, AddPaymentInfo, Purchase, CompleteRegistration, Search, ViewCategory', 'servertrack' ); ?>
                    </p>
                <?php endif; ?>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
                ...
                <?php if ( ! $cf7_active ) : ?>
                    <p class="description" style="color:#b32d2e;">
                        <?php esc_html_e( 'Contact Form 7 is not active.', 'servertrack' ); ?>
                    </p>
                <?php else : ?>
                    <p class="description">
                        <?php esc_html_e( 'Fires: Lead — configure field mappings below.', 'servertrack' ); ?>
                    </p>
                <?php endif; ?>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
                ...
                <?php if ( ! $edd_active ) : ?>
                    <p class="description" style="color:#b32d2e;">
                        <?php esc_html_e( 'Easy Digital Downloads is not active.', 'servertrack' ); ?>
                    </p>
                <?php else : ?>
                    <p class="description">
                        <?php esc_html_e( 'Fires: Purchase, ViewContent, AddToCart', 'servertrack' ); ?>
                    </p>
                <?php endif; ?>
            </td>
        </tr>
    ...
    <!-- ─────────────────────────────────────────────────────────────────── -->
    <!-- BROWSER-SIDE ENGAGEMENT TRACKING -->
    <!-- ─────────────────────────────────────────────────────────────────── -->
    <h3 style="margin-top:2.5em;"><?php esc_html_e( 'Browser Engagement Tracking', 'servertrack' ); ?></h3>
    <p class="description"><?php esc_html_e( 'These events fire in the browser pixel only (no server-side CAPI send). Useful for audience building and engagement signals.', 'servertrack' ); ?></p>
    ...
    <table class="form-table" role="presentation">
        ...
        <tr>
            ...
            <td>
                ...
                <p class="description">
                    <?php esc_html_e( 'Fires: ScrollDepth (custom) → Meta trackCustom + TikTok. Great for Content engagement audiences.', 'servertrack' ); ?>
                </p>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
                ...
                <p class="description">
                    <?php esc_html_e( 'Fires: VideoPlay, VideoProgress (custom) at 25/50/75/95% — HTML5 &lt;video&gt; and YouTube iframes.', 'servertrack' ); ?>
                </p>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
                ...
                <p class="description">
                    <?php esc_html_e( 'Fires: AddToWishlist (custom) → Meta trackCustom + TikTok.', 'servertrack' ); ?>
                </p>
            </td>
        </tr>
    ...
    <p class="description"><?php esc_html_e( 'Use any of these three methods to fire custom events to Meta CAPI + TikTok CAPI (server-side) and the browser pixel simultaneously.', 'servertrack' ); ?></p>
...
&lt;!-- Triggers: click (default) | view | hover --&gt;
&lt;div data-servertrack-event="BannerViewed"
     data-servertrack-trigger="view"
     data-servertrack-custom="1"&gt;...&lt;/div&gt;</pre>

        <h4><?php esc_html_e( '2. JavaScript API', 'servertrack' ); ?></h4>
        <pre style="background:#1e1e1e;color:#d4d4d4;padding:1em;border-radius:3px;overflow-x:auto;font-size:12px;">// Custom event (browser + server CAPI)
window.ServerTrack.track('QuizCompleted', { score: 90 });

// Standard Meta event name
window.ServerTrack.trackStandard('Lead', { content_name: 'Contact' });</pre>
    ...
    <table class="form-table" role="presentation">
        ...
        foreach ( $forms as $form ) :
            ...
            <tr>
                ...
                <td>
                    <p>
                        <label><?php esc_html_e( 'Email field name:', 'servertrack' ); ?><br>
                        <input type="text" name="servertrack_cf7_mappings[<?php echo esc_attr( $fid ); ?>][email]"
                               value="<?php echo esc_attr( $saved['email'] ?? 'your-email' ); ?>" class="regular-text" /></label>
                    </p>
                    <p>
                        <label><?php esc_html_e( 'Phone field name:', 'servertrack' ); ?><br>
                        <input type="text" name="servertrack_cf7_mappings[<?php echo esc_attr( $fid ); ?>][phone]"
                               value="<?php echo esc_attr( $saved['phone'] ?? 'your-phone' ); ?>" class="regular-text" /></label>
                    </p>
                    <p>
                        <label><?php esc_html_e( 'Name field name:', 'servertrack' ); ?><br>
                        <input type="text" name="servertrack_cf7_mappings[<?php echo esc_attr( $fid ); ?>][name]"
                               value="<?php echo esc_attr( $saved['name'] ?? 'your-name' ); ?>" class="regular-text" /></label>
                    </p>
                </td>
            </tr>
        <?php endforeach; ?>
