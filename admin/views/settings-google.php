 ...
 *
 * v3.6 fix: Removed the inner <form> / settings_fields() / submit_button() wrapper.
 * The Google tab was the only tab still using the old WP Options API form pattern,
 * which caused a double-form conflict inside render_page()'s outer wrapper and
 * completely bypassed the AJAX save path. All tabs now save via the unified
 * AJAX handler (servertrack_save_settings) through the st-save-settings button
 * rendered by render_page() in the st-settings-form-footer.
 *
 * Day 6 additions (retained):
    ...
    <div class="st-oauth-card <?php echo $is_connected ? 'st-oauth-card--connected' : 'st-oauth-card--disconnected'; ?>">
        ...
        <div class="st-oauth-card__body">
            <?php if ( $is_connected ) : ?>
                ...
                <p>
                    <a href="<?php echo esc_url( admin_url( 'admin.php?page=servertrack-settings&tab=google&st_google_action=revoke&_wpnonce=' . wp_create_nonce( 'st_google_revoke' ) ) ); ?>"
            ...
            <?php else : ?>
                ...
                $redirect_uri  = admin_url( 'admin.php?page=servertrack-settings&tab=google' );
    ...
    <div class="st-help-box">
        ...
        <ol>
            ...
            <li><?php esc_html_e( 'Under "Authorised redirect URIs", add the following URI exactly:', 'servertrack' ); ?>
                <code class="st-copy-uri"><?php echo esc_url( admin_url( 'admin.php?page=servertrack-settings&tab=google' ) ); ?></code>
