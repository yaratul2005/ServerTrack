...
class Ratul_ACT_Dashboard {
    ...
    public static function init(): void {
        ...
        add_action( 'wp_ajax_ratul_act_log_data',        [ self::class, 'ajax_log_data' ] );
        add_action( 'wp_ajax_ratul_act_platform_health', [ self::class, 'ajax_platform_health' ] );
        add_action( 'wp_ajax_ratul_act_stats_breakdown', [ self::class, 'ajax_stats_breakdown' ] );
        add_action( 'wp_ajax_ratul_act_clear_log',       [ self::class, 'ajax_clear_log' ] );
        add_action( 'wp_ajax_ratul_act_drain_retries',   [ self::class, 'ajax_drain_retries' ] );
    }
    ...
    public static function register_menu(): void {
        ...
        add_menu_page(
            __( 'Ratuls- Ads Conversion Tracker', 'ratul-ads-conversion-tracker' ),
            __( 'Ratuls-ACT', 'ratul-ads-conversion-tracker' ),
            'manage_options',
    ...
    public static function enqueue_assets( string $hook ): void {
        if ( 'toplevel_page_ratul-ads-conversion-tracker' !== $hook ) {
            return;
        }
        wp_enqueue_script(
            'chart-js',
    ...
    public static function render_page(): void {
        ...
        if ( $current_page !== '' && $current_page !== 'ratul-ads-conversion-tracker' ) {
            return;
        }
        ...
        $emq_data    = Ratul_ACT_MatchQuality::get_daily_averages( 7 );
        $platforms   = self::get_platform_statuses( $logs );
        $breakdown   = self::compute_breakdown( $logs );
        $retry_items = get_option( 'ratul_act_retry_queue', [] );
        $nonce       = wp_create_nonce( 'ratul_act_dashboard' );

        ?>
        <div class="wrap" id="ratul-ads-conversion-tracker-wrap">

        <?php Ratul_ACT_Admin::render_page_header(); ?>
        ...
        <div class="st-row">
            <div class="st-panel">
                <div class="st-panel-header">
                    ...
                    <a href="<?php echo esc_url( admin_url( 'admin.php?page=ratul-ads-conversion-tracker-settings' ) ); ?>" class="st-panel-action">Configure
                ...
                <div class="st-plat-list">
                    <?php foreach ( $platforms as $p ) :
                        ...
                        $badge_label = $enabled ? esc_html( $p['status'] ) : esc_html__( 'Disabled', 'ratul-ads-conversion-tracker' );
        ...
        <div class="st-panel">
            ...
            <div class="st-filter-bar">
                <select id="st-fp" onchange="stFilter()">
                    <option value=""><?php esc_html_e( 'All Platforms', 'ratul-ads-conversion-tracker' ); ?></option>
                    <option value="meta">Meta</option>
                    <option value="tiktok">TikTok</option>
                    <option value="google">Google</option>
                </select>
                <select id="st-fs" onchange="stFilter()">
                    <option value=""><?php esc_html_e( 'All Statuses', 'ratul-ads-conversion-tracker' ); ?></option>
        ...
        </div><!-- #ratul-ads-conversion-tracker-wrap -->
        ...
        window.addEventListener('load', function(){
            ...
            function doRefresh(){
                ...
                // Refresh log rows
                fetch(ajaxUrl+'?action=ratul_act_log_data&nonce='+encodeURIComponent(nonce))
                    ...
                    .catch(function(e){ console.error('Ratul_ACT AJAX Error:', e); })
                    .finally(function(){
                        if(btn) btn.classList.remove('st-spinning');
                        if(spinner) spinner.style.display='none';
                    });

                // FIX Bug 1 & 5: Refresh chart data via ratul_act_stats_breakdown AJAX.
                ...
                fetch(ajaxUrl+'?action=ratul_act_stats_breakdown&nonce='+encodeURIComponent(nonce))
                    .then(function(r){return r.json();})
                    .then(function(res){
            ...
            if(clearBtn){
                clearBtn.addEventListener('click', function(){
                    ...
                    fetch(ajaxUrl,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'action=ratul_act_clear_log&nonce='+encodeURIComponent(nonce)})
                    .then(function(r){return r.json();})
                    .then(function(res){
                        if(res.success){
                            var tbody = document.getElementById('st-log-tbody');
            ...
            if(drainBtn){
                drainBtn.addEventListener('click', function(){
                    ...
                    fetch(ajaxUrl,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'action=ratul_act_drain_retries&nonce='+encodeURIComponent(nonce)})
                    .then(function(r){return r.json();})
                    .then(function(res){ drainBtn.textContent = res.success ? 'Done' : 'Error'; })
                    .catch(function(e){ console.error('Ratul_ACT AJAX Error:', e); drainBtn.textContent = 'Error'; });
                });
            }
        });
        </script>
    ...
    private static function compute_stats( array $logs ): array {
        ...
        return [
            ...
            'retry_queue'  => count( get_option( 'ratul_act_retry_queue', [] ) ),
        ];
    }
    ...
    private static function get_platform_statuses( array $logs ): array {
        ...
        $defs = [
            'meta'   => [ 'name' => 'Meta CAPI',     'enabled' => (bool) get_option( 'ratul_act_meta_enabled', 0 ),   'ok' => (bool) ( get_option( 'ratul_act_meta_pixel_id', '' ) && get_option( 'ratul_act_meta_access_token', '' ) ) ],
            'google' => [ 'name' => 'Google Ads',    'enabled' => (bool) get_option( 'ratul_act_google_enabled', 0 ), 'ok' => (bool) get_option( 'ratul_act_google_refresh_token', '' ) ],
            'tiktok' => [ 'name' => 'TikTok Events', 'enabled' => (bool) get_option( 'ratul_act_tiktok_enabled', 0 ), 'ok' => (bool) ( get_option( 'ratul_act_tiktok_pixel_id', '' ) && get_option( 'ratul_act_tiktok_access_token', '' ) ) ],
        ];
        $out = [];
    ...
    public static function ajax_log_data(): void {
        check_ajax_referer( 'ratul_act_dashboard', 'nonce' );
    ...
    public static function ajax_platform_health(): void {
        check_ajax_referer( 'ratul_act_dashboard', 'nonce' );
        if ( ! current_user_can( 'manage_options' ) ) wp_send_json_error( 'Unauthorized' );
        wp_send_json_success( self::get_platform_statuses( get_option( 'ratul_act_debug_log', [] ) ) );
    }

    public static function ajax_stats_breakdown(): void {
        check_ajax_referer( 'ratul_act_dashboard', 'nonce' );
        if ( ! current_user_can( 'manage_options' ) ) wp_send_json_error( 'Unauthorized' );
        wp_send_json_success( self::compute_breakdown( get_option( 'ratul_act_debug_log', [] ) ) );
    }

    public static function ajax_clear_log(): void {
        check_ajax_referer( 'ratul_act_dashboard', 'nonce' );
        if ( ! current_user_can( 'manage_options' ) ) wp_send_json_error( 'Unauthorized' );
        Ratul_ACT_Logger::clear_logs();
    ...
    public static function ajax_drain_retries(): void {
        check_ajax_referer( 'ratul_act_dashboard', 'nonce' );
        if ( ! current_user_can( 'manage_options' ) ) wp_send_json_error( 'Unauthorized' );
        $queue = get_option( 'ratul_act_retry_queue', [] );
        if ( empty( $queue ) ) { wp_send_json_success( [ 'drained' => 0 ] ); return; }
        $drained = 0;
        foreach ( $queue as $item ) {
            if ( class_exists( 'Ratul_ACT_Retry' ) ) { Ratul_ACT_Retry::process_item( $item ); $drained++; }
        }
        delete_option( 'ratul_act_retry_queue' );
