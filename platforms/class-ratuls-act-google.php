...
class Ratuls_ACT_Google {
    ...
    public static function send( Ratuls_ACT_Event $event ): array {
        ...
        $customer_id   = get_option( 'ratuls_act_google_customer_id', '' );
        $conversion_id = get_option( 'ratuls_act_google_conversion_id', '' );
        $conv_label    = get_option( 'ratuls_act_google_conversion_label', '' );

        if ( ! $customer_id || ! $conversion_id || ! $conv_label ) {
            Ratuls_ACT_Logger::log( 'error', 'google', 'Missing Google Ads credentials (customer_id / conversion_id / label).' );
        ...
        $response = wp_remote_post( $endpoint, [
            ...
            'headers' => [
                ...
                'developer-token'   => get_option( 'ratuls_act_google_developer_token', '' ),
    ...
    private static function get_access_token(): string {
        $token   = (string) get_option( 'ratuls_act_google_access_token', '' );
        $expires = (int) get_option( 'ratuls_act_google_token_expires', 0 );
    ...
    private static function refresh_access_token(): string {
        ...
        $client_secret = get_option( 'ratuls_act_google_client_secret', '' );
        $refresh_token = get_option( 'ratuls_act_google_refresh_token', '' );

        if ( empty( $client_id ) || empty( $client_secret ) || empty( $refresh_token ) ) {
            Ratuls_ACT_Logger::log( 'error', 'google', 'OAuth credentials missing — cannot refresh token.' );
            return '';
        }
        ...
        if ( is_wp_error( $response ) ) {
            Ratuls_ACT_Logger::log( 'error', 'google', 'Token refresh failed: ' . $response->get_error_message() );
            return '';
        }
        ...
        update_option( 'ratuls_act_google_access_token', $new_token );
        update_option( 'ratuls_act_google_token_expires', time() + $expires_in );

        return $new_token;
    }

    private static function build_payload( Ratuls_ACT_Event $event, string $conversion_id, string $conv_label ): array {
        ...
        if ( $order_id > 0 ) {
            $consent_granted = Ratuls_ACT_Consent::is_granted( 'google', $order_id );
        } else {
            // Offline conversions default to true (store operator responsibility)
            $consent_granted = apply_filters( 'ratuls_act_offline_consent_default', true );
        }
