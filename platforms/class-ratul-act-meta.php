 ...
 * Depends on: Ratul_ACT_Event, Ratul_ACT_Logger
 *
 * Changes in v2.3 (FIX-05):
 *   Added platform-enabled guard at the top of send().
 *   Google::send() already had this guard (get_option ratul_act_google_enabled).
 ...
 *     Ratul_ACT_MatchQuality::score() and passed directly to Logger there.
 *
...
class Ratul_ACT_Meta {
    ...
    public static function send( Ratul_ACT_Event $event ): array {
        ...
        $pixels_json = get_option( 'ratul_act_meta_pixels_array', '' );
        $pixels = [];
        ...
        if ( empty( $pixels ) ) {
            $pixel_id     = trim( (string) get_option( 'ratul_act_meta_pixel_id', '' ) );
            $access_token = trim( (string) get_option( 'ratul_act_meta_access_token', '' ) );
        ...
        $test_code = ! empty( $event->custom_data['_test_event_code'] )
            ...
            : trim( (string) get_option( 'ratul_act_meta_test_event_code', '' ) );
