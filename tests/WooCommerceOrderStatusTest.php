...
class WooCommerceOrderStatusTest extends TestCase {
    ...
    public function test_on_hold_fires_lead(): void {
        Ratuls_ACT_Source_WooCommerce::handle_order_status_change( 101, 'pending', 'on-hold', $this->order );

        $this->assertCount( 1, Ratuls_ACT_Core::$dispatched );
        $this->assertSame( 'Lead', Ratuls_ACT_Core::$dispatched[0]['event'] );
    }

    public function test_failed_fires_contact(): void {
        Ratuls_ACT_Source_WooCommerce::handle_order_status_change( 101, 'pending', 'failed', $this->order );

        $this->assertCount( 1, Ratuls_ACT_Core::$dispatched );
        $this->assertSame( 'Contact', Ratuls_ACT_Core::$dispatched[0]['event'] );
    }

    public function test_cancelled_fires_submit_form(): void {
        Ratuls_ACT_Source_WooCommerce::handle_order_status_change( 101, 'processing', 'cancelled', $this->order );

        $this->assertCount( 1, Ratuls_ACT_Core::$dispatched );
        $this->assertSame( 'SubmitForm', Ratuls_ACT_Core::$dispatched[0]['event'] );
    }

    public function test_unknown_status_fires_nothing(): void {
        Ratuls_ACT_Source_WooCommerce::handle_order_status_change( 101, 'pending', 'refunded', $this->order );

        $this->assertEmpty( Ratuls_ACT_Core::$dispatched, 'Unknown status must not dispatch any event.' );
    }
    ...
    public function test_dedup_key_format_is_correct(): void {
        Ratuls_ACT_Source_WooCommerce::handle_order_status_change( 202, 'pending', 'cancelled', $this->order );

        $dispatched = Ratuls_ACT_Core::$dispatched[0];
