<?php
/**
 * Tests for Ratul_ACT_Retry v2.2
 *
 * Covers:
...
class RetryTest extends TestCase {
    ...
    public function test_maybe_queue_queues_network_error(): void {
        ...
        Ratul_ACT_Retry::maybe_queue( 'meta', $result, $event_args );

        $queue = get_option( Ratul_ACT_Retry::QUEUE_OPTION, [] );
    ...
    public function test_maybe_queue_queues_500_error(): void {
        ...
        Ratul_ACT_Retry::maybe_queue( 'tiktok', $result, $event_args );
    ...
    public function test_maybe_queue_queues_429_rate_limit(): void {
        ...
        Ratul_ACT_Retry::maybe_queue( 'google', $result, $event_args );
    ...
    public function test_maybe_queue_deduplicates_same_event(): void {
        ...
        Ratul_ACT_Retry::maybe_queue( 'meta', $result, $event_args );
        Ratul_ACT_Retry::maybe_queue( 'meta', $result, $event_args ); // duplicate
    ...
    public function test_process_skips_items_not_yet_due(): void {
        ...
        update_option( Ratul_ACT_Retry::QUEUE_OPTION, $queue );

        Ratul_ACT_Retry::process();

        $remaining = get_option( Ratul_ACT_Retry::QUEUE_OPTION, [] );
    ...
    public function test_process_marks_dedup_for_order_id_on_success(): void {
        ...
        $this->assertTrue(
            Ratul_ACT_Dedup::already_sent( 99, 'meta' ),
    ...
    public function test_process_abandons_item_after_max_attempts(): void {
        $queue = [
            'uid_max' => [
                ...
                'attempts'     => Ratul_ACT_Retry::MAX_ATTEMPTS,  // already at max
                'next_retry'   => time() - 1,
                'queued_at'    => time() - 3600,
                'last_attempt' => null,
            ],
        ...
        $warnings = array_filter( Ratul_ACT_Logger::$log, fn( $e ) => $e['level'] === 'warning' );
    ...
    public function test_process_updates_last_attempt_timestamp(): void {
        ...
        $info = array_filter( Ratul_ACT_Logger::$log, fn( $e ) => $e['level'] === 'info' );
    ...
    public function test_process_queue_delegates_to_process(): void {
        ...
        Ratul_ACT_Retry::process_queue(); // calls process() internally
    ...
    public function test_event_to_args_carries_dedup_key_when_present(): void {
        $event = new Ratul_ACT_Event( 'InitiateCheckout', 'eid_001' );
        $event->set_custom_data( [ '_dedup_key' => 'abandon_xyz', 'value' => 49.99 ] );

        $args = Ratul_ACT_Retry::event_to_args( $event );
    ...
    public function test_event_to_args_omits_dedup_key_when_absent(): void {
        $event = new Ratul_ACT_Event( 'Purchase', 'eid_002' );
    ...
    public function test_event_to_args_preserves_event_name_and_id(): void {
        $event = new Ratul_ACT_Event( 'AddToCart', 'eid_003' );
        $args  = Ratul_ACT_Retry::event_to_args( $event );
