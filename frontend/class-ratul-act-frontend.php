...
class Ratul_ACT_Frontend {
    ...
    public static function register_rest_routes() {
        register_rest_route( 'ratul-ads-conversion-tracker/v1', '/capture-clickids', [
            'methods'  => 'POST',
        ...
        register_rest_route( 'ratul-ads-conversion-tracker/v1', '/capture-pii', [
            'methods'  => 'POST',
            'callback' => function( WP_REST_Request $req ) {
                ...
                if ( $em ) {
                    if ( ! $is_hashed ) {
                        $em = Ratul_ACT_Hasher::hash_email( $em );
                    }
                    if ( function_exists( 'WC' ) && WC()->session ) {
        ...
        register_rest_route( 'ratul-ads-conversion-tracker/v1', '/custom-event', [
            'methods'             => 'POST',
    ...
    public static function rest_custom_event( WP_REST_Request $request ) {
        if ( ! get_option( 'ratul_act_enabled', 1 ) ) {
            return new WP_Error( 'disabled', 'Ratul_ACT disabled', [ 'status' => 403 ] );
        }
        ...
        $event_id = $request->get_param( 'event_id' ) ?: Ratul_ACT_Dedup::generate_event_id( $event_name . '_rest_' . time() );
        ...
        // from appearing in the Ratul_ACT debug log table.
        ...
        if ( is_user_logged_in() ) {
            ...
            if ( $user->user_email ) $user_data['email'] = Ratul_ACT_Hasher::hash_email( $user->user_email );
        }
        ...
        if ( get_option( 'ratul_act_meta_enabled', 0 ) && Ratul_ACT_Consent::is_granted( 'meta' ) ) {
            $results['meta'] = Ratul_ACT_Meta::send( $event );
        }
        if ( get_option( 'ratul_act_tiktok_enabled', 0 ) && Ratul_ACT_Consent::is_granted( 'tiktok' ) ) {
            $results['tiktok'] = Ratul_ACT_TikTok::send( $event );
        }
    ...
    public static function enqueue_pixel_script() {
        if ( is_admin() || ! get_option( 'ratul_act_enabled', 1 ) ) return;
        ...
        $config = [
            'meta_pixel'     => get_option( 'ratul_act_meta_pixel_id', '' ),
            'tiktok_pixel'   => get_option( 'ratul_act_tiktok_pixel_id', '' ),
            'test_mode'      => (bool) get_option( 'ratul_act_test_mode', 0 ),
            'meta_enabled'   => (bool) get_option( 'ratul_act_meta_enabled', 0 ),
            'tt_enabled'     => (bool) get_option( 'ratul_act_tiktok_enabled', 0 ),
            'google_enabled' => (bool) get_option( 'ratul_act_google_enabled', 0 ),
            'gtag_id'        => get_option( 'ratul_act_google_gtag_id', '' ),
            'gtag_label'     => get_option( 'ratul_act_google_gtag_label', '' ),
            'store_currency' => function_exists( 'get_woocommerce_currency' ) ? get_woocommerce_currency() : 'USD',
            'scroll_depth_enabled'   => (bool) get_option( 'ratul_act_scroll_depth', 1 ),
            'video_tracking_enabled' => (bool) get_option( 'ratul_act_video_tracking', 1 ),
            'wishlist_enabled'       => (bool) get_option( 'ratul_act_wishlist_tracking', 1 ),
            'is_product'         => false,
        ...
        if ( is_user_logged_in() ) {
            $user = wp_get_current_user();
            $user_data_for_pixel = [
                'em' => $user->user_email ? Ratul_ACT_Hasher::hash_email( $user->user_email ) : '',
                'fn' => $user->first_name ? Ratul_ACT_Hasher::hash_name( $user->first_name ) : '',
                'ln' => $user->last_name ? Ratul_ACT_Hasher::hash_name( $user->last_name ) : '',
                'ph' => get_user_meta( $user->ID, 'billing_phone', true ) ? Ratul_ACT_Hasher::hash_phone( get_user_meta( $user->ID, 'billing_phone', true ) ) : '',
                'ct' => get_user_meta( $user->ID, 'billing_city', true ) ? Ratul_ACT_Hasher::hash_city( get_user_meta( $user->ID, 'billing_city', true ) ) : '',
                'st' => get_user_meta( $user->ID, 'billing_state', true ) ? Ratul_ACT_Hasher::hash_state( get_user_meta( $user->ID, 'billing_state', true ) ) : '',
                'zp' => get_user_meta( $user->ID, 'billing_postcode', true ) ? Ratul_ACT_Hasher::hash_zip( get_user_meta( $user->ID, 'billing_postcode', true ) ) : '',
                'country' => get_user_meta( $user->ID, 'billing_country', true ) ? Ratul_ACT_Hasher::hash_country( get_user_meta( $user->ID, 'billing_country', true ) ) : '',
                'external_id' => Ratul_ACT_Identity::get_external_id_for_user( $user->ID ),
            ];
        ...
        $config['event_ids'] = [
            'PageView' => Ratul_ACT_Hasher::event_id('PageView', $session_key),
            'ViewContent' => Ratul_ACT_Hasher::event_id('ViewContent', $session_key),
            'AddToCart' => Ratul_ACT_Hasher::event_id('AddToCart', $session_key),
            'InitiateCheckout' => Ratul_ACT_Hasher::event_id('InitiateCheckout', $session_key),
            'AddPaymentInfo' => Ratul_ACT_Hasher::event_id('AddPaymentInfo', $session_key)
        ];

        $config['rest_url'] = rest_url( 'ratul-ads-conversion-tracker/v1/' );
        ...
        if ( function_exists( 'is_product_category' ) && is_product_category() ) {
            ...
            $config['is_product_archive'] = true;
            $config['current_category']   = __( 'Shop', 'ratul-ads-conversion-tracker' );
        }
        ...
        if ( function_exists( 'is_checkout' ) && is_checkout() && ! is_order_received_page() ) {
            ...
            if ( function_exists( 'WC' ) && WC()->session ) {
                WC()->session->set( 'ratul_act_ic_event_id', $event_id );
            }
        }
        ...
        // ── Thank-you / Purchase ──────────────────────────────────────────────
        if ( function_exists( 'is_order_received_page' ) && is_order_received_page() && ! get_option( 'ratul_act_manual_purchase_enabled', 0 ) ) {
            ...
            if ( $order_id ) {
                ...
                if ( $order ) {
                    ...
                    if ( empty( $event_id ) ) {
                        $event_id = Ratul_ACT_PixelDedup::get_order_event_id( $order_id, 'purchase' );
                        if ( ! empty( $event_id ) ) {
                    ...
                    $gclid = (string) $order->get_meta( '_ratul_act_gclid' );
        ...
        wp_localize_script( 'ratul-ads-conversion-tracker-pixel', 'ratul_act_config', $config );
        wp_enqueue_script( 'ratul-ads-conversion-tracker-pixel' );
    }
    ...
    public static function capture_click_ids() {
        ...
        if ( ! empty( $_GET['fbclid'] ) ) {
            ...
            if ( $fingerprint ) {
                set_transient( 'ratul_act_fbc_' . $fingerprint, $fbc, 90 * DAY_IN_SECONDS );
                set_transient( 'ratul_act_fbclid_' . $fingerprint, $fbclid, 90 * DAY_IN_SECONDS );
            }
        ...
        if ( ! empty( $_GET['gclid'] ) ) {
            ...
            if ( $fingerprint ) set_transient( 'ratul_act_gclid_' . $fingerprint, $gclid, 90 * DAY_IN_SECONDS );
        ...
        if ( ! empty( $_GET['ttclid'] ) ) {
            ...
            if ( $fingerprint ) set_transient( 'ratul_act_ttclid_' . $fingerprint, $ttclid, 7 * DAY_IN_SECONDS );
    ...
    public static function persist_click_ids_to_order( WC_Order $order ) {
        ...
        if ( ! empty( $_COOKIE['_fbc'] ) ) { // phpcs:ignore
            ...
            $fbc = (string) get_transient( 'ratul_act_fbc_' . $fingerprint );
        }
        if ( $fbc ) $order->update_meta_data( '_ratul_act_fbc', $fbc );

        if ( $fingerprint ) {
            $fbclid = (string) get_transient( 'ratul_act_fbclid_' . $fingerprint );
            if ( $fbclid ) $order->update_meta_data( '_ratul_act_fbclid', $fbclid );
        }

        if ( ! empty( $_COOKIE['_fbp'] ) ) { // phpcs:ignore
            $order->update_meta_data( '_ratul_act_fbp', sanitize_text_field( wp_unslash( $_COOKIE['_fbp'] ) ) );
        }
        ...
        if ( ! empty( $_COOKIE['ttclid'] ) ) { // phpcs:ignore
            ...
            $ttclid = (string) get_transient( 'ratul_act_ttclid_' . $fingerprint );
        }
        if ( $ttclid ) $order->update_meta_data( '_ratul_act_ttclid', $ttclid );

        $gclid = '';
        if ( ! empty( $_COOKIE['_gcl_aw'] ) ) { // phpcs:ignore
            $gclid = sanitize_text_field( wp_unslash( $_COOKIE['_gcl_aw'] ) );
        } elseif ( $fingerprint ) {
            $gclid = (string) get_transient( 'ratul_act_gclid_' . $fingerprint );
        }
