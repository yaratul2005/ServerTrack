# Ratul_ACT Core Modules & Feature Highlights

## 1. 1st-Party CAPI Gateway (Pixel Proxy)
Traditional trackers drop payloads from the browser directly to external domains (like `analytics.tiktok.com`). This fails inherently against network ad-blockers and privacy extensions. Ratul_ACT solves this via an internal REST bridge, safely proxying browser beacons directly into the PHP engine to maintain seamless data fidelity.

## 2. Server-Side Cookie Survival (ITP Defeat)
Safari sets a strict 7-day expiration limit on cookies created via `document.cookie`. This destroys remarketing attribution for iOS/macOS users. Ratul_ACT intercepts click IDs (`fbclid`, `gclid`) server-side prior to rendering HTML and enforces HTTP-only `Set-Cookie` directives, elevating the cookie's lifespan to 2 years.
...
Conversion APIs live and die by match scoring. Ratul_ACT strips and standardizes HTTP properties (IP, UA) via advanced fallback resolution (circumventing Cloudflare IP collapse issues) while safely hashing sensitive PII in real-time.
...
Tracking failures usually happen silently when tokens expire or are revoked. Ratul_ACT features an internal WP-Cron validation heartbeat that pings diagnostic endpoints daily and triggers direct email alerts to store administrators upon authentication failure.
