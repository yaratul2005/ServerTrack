 ...
 *   page events. Key: ratuls_act_wl_{user_hash}_{product_id}, TTL 60 minutes.
 ...
 *
 * OPT-IN:
 *   Only active when ratuls_act_source_wishlist_enabled = 1 (default: 0).
 *   Admin can toggle in Settings → Sources.
 *
 * @package Ratuls_ACT
 * @since   5.0.0
 */
class Ratuls_ACT_WooWishlist {
    ...
    public static function init(): void {
        ...
        // Async handler
        add_action( 'ratuls_act_send_wishlist_add',                 [ self::class, 'send_wishlist_async' ],   10, 1 );
    }
    ...
    private static function maybe_queue( int $product_id ): void {
        ...
        $transient_key = 'ratuls_act_wl_' . $user_hash . '_' . $product_id;
        ...
        if ( is_user_logged_in() ) {
            ...
            if ( $user->user_email ) {
                $context['email']       = Ratuls_ACT_Hasher::hash_email( $user->user_email );
                $context['external_id'] = Ratuls_ACT_Identity::get_external_id_for_user( $user->ID );
            }
        }

        wp_schedule_single_event(
            time(),
            'ratuls_act_send_wishlist_add',
    ...
    public static function send_wishlist_async( array $args ): void {
        ...
        $event_id = Ratuls_ACT_Dedup::generate_event_id( 'wishlist_' . $product_id );

        $custom_data = apply_filters( 'ratuls_act_wishlist_custom_data', [
        ...
        if ( get_option( 'ratuls_act_meta_enabled', 0 )
            && Ratuls_ACT_Consent::is_granted( 'meta' ) ) {
            $r = Ratuls_ACT_Meta::send( $event );
            if ( ( $r['status'] ?? '' ) !== 'success' ) {
                Ratuls_ACT_Retry::maybe_queue( 'meta', $r, Ratuls_ACT_Retry::event_to_args( $event ) );
            }
            Ratuls_ACT_Logger::log( $r['status'] ?? 'error', 'meta', 'AddToWishlist #' . $product_id, '', $event_id, 0, 'AddToWishlist' );
        }

        // TIKTOK
        if ( get_option( 'ratuls_act_tiktok_enabled', 0 )
            && Ratuls_ACT_Consent::is_granted( 'tiktok' ) ) {
            $r = Ratuls_ACT_TikTok::send( $event );
            if ( ( $r['status'] ?? '' ) !== 'success' ) {
                Ratuls_ACT_Retry::maybe_queue( 'tiktok', $r, Ratuls_ACT_Retry::event_to_args( $event ) );
            }
            Ratuls_ACT_Logger::log( $r['status'] ?? 'error', 'tiktok', 'AddToWishlist #' . $product_id, '', $event_id, 0, 'AddToWishlist' );
        }

        // Google: no standard AddToWishlist event — log as skipped
        if ( get_option( 'ratuls_act_google_enabled', 0 ) ) {
            Ratuls_ACT_Logger::log( 'skipped', 'google', 'AddToWishlist: no Google standard event — skipped.', '', $event_id, 0, 'AddToWishlist' );
        }
    }
