...
/**
 * Ratuls_ACT_WooPartialRefund  v1.0
 *
 * Sends server-side CAPI events for WooCommerce partial refunds.
 *
 * WHY A SEPARATE CLASS:
 *   The main WooCommerce source (class-ratuls-act-woocommerce.php) handles
 ...
 * @since   5.0.0
 */
class Ratuls_ACT_WooPartialRefund {
    ...
    public static function init(): void {
        ...
        add_action(
            'ratuls_act_send_partial_refund',
    ...
    public static function on_partial_refund( int $order_id, int $refund_id ): void {
        ...
        $meta_done   = ! get_option( 'ratuls_act_meta_enabled', 0 )   || Ratuls_ACT_Dedup::exists( $base_key . '_meta' );
        $tiktok_done = ! get_option( 'ratuls_act_tiktok_enabled', 0 ) || Ratuls_ACT_Dedup::exists( $base_key . '_tiktok' );
        $google_done = ! get_option( 'ratuls_act_google_enabled', 0 ) || Ratuls_ACT_Dedup::exists( $base_key . '_google' );
    ...
    public static function send_partial_refund_async( int $order_id, int $refund_id ): void {
        ...
        if ( ! $order ) {
            Ratuls_ACT_Logger::log( 'error', 'all', 'Partial refund: order #' . $order_id . ' not found.', '', '', $order_id, 'PartialRefund' );
            return;
        }
        ...
        if ( ! $refund ) {
            Ratuls_ACT_Logger::log( 'error', 'all', 'Partial refund: refund #' . $refund_id . ' not found.', '', '', $order_id, 'PartialRefund' );
            return;
        }
        ...
        if ( $refund_amount <= 0 ) {
            Ratuls_ACT_Logger::log( 'skipped', 'all', 'Partial refund #' . $refund_id . ' amount is zero — skipped.', '', '', $order_id, 'PartialRefund' );
            return;
        }
        ...
        if ( get_option( 'ratuls_act_meta_enabled', 0 )
            && ! Ratuls_ACT_Dedup::exists( $base_key . '_meta' )
            ...
            if ( ( $r['status'] ?? '' ) === 'success' ) {
                Ratuls_ACT_Dedup::set( $base_key . '_meta' );
            } else {
                Ratuls_ACT_Retry::maybe_queue( 'meta', $r, Ratuls_ACT_Retry::event_to_args( $e ) );
            }
            Ratuls_ACT_Logger::log( $r['status'] ?? 'error', 'meta', 'PartialRefund #' . $refund_id . ' / order #' . $order_id, '', $event_id, $order_id, 'PartialRefund', $emq );
        }

        // ── TIKTOK ───────────────────────────────────────────────────────────
        if ( get_option( 'ratuls_act_tiktok_enabled', 0 )
            && ! Ratuls_ACT_Dedup::exists( $base_key . '_tiktok' )
            && Ratuls_ACT_Consent::is_granted( 'tiktok', $order_id ) ) {
            $e = ( new Ratuls_ACT_Event( 'PlaceAnOrder', $event_id ) )
                ->set_user_data( $user_data )
                ->set_custom_data( $custom_data );
            $r = Ratuls_ACT_TikTok::send( $e );
            if ( ( $r['status'] ?? '' ) === 'success' ) {
                Ratuls_ACT_Dedup::set( $base_key . '_tiktok' );
            } else {
                Ratuls_ACT_Retry::maybe_queue( 'tiktok', $r, Ratuls_ACT_Retry::event_to_args( $e ) );
            }
            Ratuls_ACT_Logger::log( $r['status'] ?? 'error', 'tiktok', 'PartialRefund #' . $refund_id . ' / order #' . $order_id, '', $event_id, $order_id, 'PartialRefund', $emq );
        }

        // ── GOOGLE ───────────────────────────────────────────────────────────
        if ( get_option( 'ratuls_act_google_enabled', 0 )
            && ! Ratuls_ACT_Dedup::exists( $base_key . '_google' )
            && Ratuls_ACT_Consent::is_granted( 'google', $order_id ) ) {
            $e = ( new Ratuls_ACT_Event( 'refund', $event_id ) )
                ->set_user_data( $user_data )
                ->set_custom_data( $custom_data );
            $r = Ratuls_ACT_Google::send( $e );
            if ( ( $r['status'] ?? '' ) === 'success' ) {
                Ratuls_ACT_Dedup::set( $base_key . '_google' );
            } else {
                Ratuls_ACT_Retry::maybe_queue( 'google', $r, Ratuls_ACT_Retry::event_to_args( $e ) );
            }
            Ratuls_ACT_Logger::log( $r['status'] ?? 'error', 'google', 'PartialRefund #' . $refund_id . ' / order #' . $order_id, '', $event_id, $order_id, 'PartialRefund', $emq );
        }
    }
    ...
    private static function build_user_data( WC_Order $order ): array {
        ...
        foreach ( [
            ...
            if ( $val ) $data[ $key ] = Ratuls_ACT_Hasher::hash( (string) $val );
        }
