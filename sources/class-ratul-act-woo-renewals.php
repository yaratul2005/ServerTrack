...
class Ratul_ACT_WooRenewals {
    ...
    public static function init() {
        if ( ! get_option( 'ratul_act_source_woo_enabled', 1 ) ) return;
        ...
        add_action( 'ratul_act_send_renewal_purchase',      [ self::class, 'send_renewal_async' ],       10, 1 );
        add_action( 'ratul_act_send_subscription_cancelled', [ self::class, 'send_cancellation_async' ], 10, 1 );
    }
    ...
    public static function on_renewal_complete( $subscription, $renewal_order ) {
        ...
        if ( Ratul_ACT_Dedup::was_sent( $renewal_order_id, 'meta' )
            || Ratul_ACT_Dedup::was_sent( $renewal_order_id, 'google' )
            || Ratul_ACT_Dedup::was_sent( $renewal_order_id, 'tiktok' ) ) {
            Ratul_ACT_Logger::log( 'dedup_blocked', 'all', 'Renewal order #' . $renewal_order_id . ' already sent — skipping.', '', Ratul_ACT_Dedup::get_event_id( $renewal_order_id ), $renewal_order_id, 'Purchase' );
            return;
        }
        $event_id = Ratul_ACT_Dedup::generate_event_id( 'renewal_' . $renewal_order_id );
        Ratul_ACT_Dedup::store_event_id( $renewal_order_id, $event_id );
        Ratul_ACT_Logger::log( 'queued', 'all', 'Subscription renewal order #' . $renewal_order_id . ' queued for server-side tracking.', '', $event_id, $renewal_order_id, 'Purchase' );
        wp_schedule_single_event( time(), 'ratul_act_send_renewal_purchase', [ $renewal_order_id ] );
    }
    ...
    public static function send_renewal_async( int $renewal_order_id ) {
        ...
        if ( ! $order ) {
            Ratul_ACT_Logger::log( 'error', 'all', 'Renewal order #' . $renewal_order_id . ' not found in cron.', '', '', $renewal_order_id, 'Purchase' );
            return;
        }
        $event_id    = Ratul_ACT_Dedup::get_event_id( $renewal_order_id );
        ...
        if ( get_option( 'ratul_act_meta_enabled', 0 ) ) {
            if ( ! Ratul_ACT_Dedup::was_sent( $renewal_order_id, 'meta' ) ) {
                ...
                if ( ( $result['status'] ?? '' ) === 'success' ) {
                    Ratul_ACT_Dedup::mark_as_sent( $renewal_order_id, 'meta' );
                } else {
        ...
        if ( get_option( 'ratul_act_google_enabled', 0 ) ) {
            if ( ! Ratul_ACT_Dedup::was_sent( $renewal_order_id, 'google' ) ) {
                $e      = ( new Ratul_ACT_Event( 'Purchase', $event_id ) )->set_user_data( $user_data )->set_custom_data( $custom_data );
                $result = Ratul_ACT_Google::send( $e );
                if ( ( $result['status'] ?? '' ) === 'success' ) {
                    Ratul_ACT_Dedup::mark_as_sent( $renewal_order_id, 'google' );
                } else {
        ...
        if ( get_option( 'ratul_act_tiktok_enabled', 0 ) ) {
            if ( ! Ratul_ACT_Dedup::was_sent( $renewal_order_id, 'tiktok' ) ) {
                $e      = ( new Ratul_ACT_Event( 'Purchase', $event_id ) )->set_user_data( $user_data )->set_custom_data( $custom_data );
                $result = Ratul_ACT_TikTok::send( $e );
                if ( ( $result['status'] ?? '' ) === 'success' ) {
                    Ratul_ACT_Dedup::mark_as_sent( $renewal_order_id, 'tiktok' );
                } else {
    ...
    public static function on_subscription_cancelled( $subscription ) {
        ...
        if ( Ratul_ACT_Dedup::was_sent( Ratul_ACT_Subscriptions::SUBSCRIPTION_CANCEL_DEDUP_KEY . $subscription_id, 'meta' )
            || Ratul_ACT_Dedup::was_sent( Ratul_ACT_Subscriptions::SUBSCRIPTION_CANCEL_DEDUP_KEY . $subscription_id, 'tiktok' )
            || Ratul_ACT_Dedup::was_sent( Ratul_ACT_Subscriptions::SUBSCRIPTION_CANCEL_DEDUP_KEY . $subscription_id, 'google' ) ) {
            return;
        }
        ...
        wp_schedule_single_event(
            time(),
            'ratul_act_send_subscription_cancelled',
            [ $subscription_id ]
        );
        spawn_cron();
    }
    ...
    public static function send_cancellation_async( int $subscription_id ) {
        ...
        if ( ! $subscription ) {
            Ratul_ACT_Logger::log( 'error', 'all', 'Subscription #' . $subscription_id . ' not found in cron.', '', '', $subscription_id, 'SubscriptionCancelled' );
            return;
        }
        ...
        if ( ! $last_order instanceof WC_Order ) {
            Ratul_ACT_Logger::log( 'error', 'all', 'Subscription #' . $subscription_id . ' has no order — cannot build user_data.', '', '', $subscription_id, 'SubscriptionCancelled' );
            return;
        }

        $dedup_key = Ratul_ACT_Subscriptions::SUBSCRIPTION_CANCEL_DEDUP_KEY . $subscription_id;
        $event_id  = Ratul_ACT_Dedup::get_event_id( $dedup_key );
        if ( empty( $event_id ) ) {
            $event_id = Ratul_ACT_Dedup::generate_event_id( $dedup_key );
            Ratul_ACT_Dedup::store_event_id( $dedup_key, $event_id );
        }
        ...
        if ( get_option( 'ratul_act_meta_enabled', 0 )
            && ! Ratul_ACT_Dedup::was_sent( $dedup_key, 'meta' ) ) {
            $e = ( new Ratul_ACT_Event( 'Lead', $event_id ) )
                ->set_user_data( $user_data )
                ->set_custom_data( $custom_data );
            $result = Ratul_ACT_Meta::send( $e );
            if ( ( $result['status'] ?? '' ) === 'success' ) {
                Ratul_ACT_Dedup::mark_as_sent( $dedup_key, 'meta' );
            } else {
                Ratul_ACT_Retry::maybe_queue( 'meta', $result, Ratul_ACT_Retry::event_to_args( $e ) );
            }
        }
        ...
        if ( get_option( 'ratul_act_tiktok_enabled', 0 )
            && ! Ratul_ACT_Dedup::was_sent( $dedup_key, 'tiktok' ) ) {
            $e = ( new Ratul_ACT_Event( 'Subscribe', $event_id ) )
                ->set_user_data( $user_data )
                ->set_custom_data( array_merge( $custom_data, [ 'content_name' => 'SubscriptionCancelled' ] ) );
            $result = Ratul_ACT_TikTok::send( $e );
            if ( ( $result['status'] ?? '' ) === 'success' ) {
                Ratul_ACT_Dedup::mark_as_sent( $dedup_key, 'tiktok' );
            } else {
                Ratul_ACT_Retry::maybe_queue( 'tiktok', $result, Ratul_ACT_Retry::event_to_args( $e ) );
            }
        }
        ...
        if ( get_option( 'ratul_act_google_enabled', 0 )
            && ! Ratul_ACT_Dedup::was_sent( $dedup_key, 'google' ) ) {
            $e = ( new Ratul_ACT_Event( 'subscription_cancelled', $event_id ) )
                ->set_user_data( $user_data )
                ->set_custom_data( $custom_data );
            $result = Ratul_ACT_Google::send( $e );
            if ( ( $result['status'] ?? '' ) === 'success' ) {
                Ratul_ACT_Dedup::mark_as_sent( $dedup_key, 'google' );
            } else {
                Ratul_ACT_Retry::maybe_queue( 'google', $result, Ratul_ACT_Retry::event_to_args( $e ) );
            }
        }
    ...
    private static function build_renewal_user_data( WC_Order $order ): array {
        ...
        if ( ! empty( $email ) ) $data['email'] = Ratul_ACT_Hasher::hash_email( $email );
        ...
        foreach ( $pii_map as $key => $val ) {
            if ( ! empty( $val ) ) $data[ $key ] = Ratul_ACT_Hasher::hash( $val );
        }
        $raw_map = [
        ...
        $data['external_id'] = Ratul_ACT_Hasher::hash( (string) ( $customer_id ?: $order->get_id() ) );
        return $data;
    }
