...
class Ratul_ACT_EDD {
    ...
    public static function init() {
        if ( ! get_option( 'ratul_act_source_edd_enabled', 0 ) ) return;
        ...
        add_action( 'ratul_act_send_edd_purchase', [ self::class, 'send_purchase_async' ],  10, 1 );
    }
    ...
    public static function on_purchase( int $payment_id ) {
        ...
        $event_id = Ratul_ACT_Dedup::generate_event_id( 'edd_purchase_' . $payment_id );
        Ratul_ACT_Dedup::store_event_id( $payment_id, $event_id );

        wp_schedule_single_event( time(), 'ratul_act_send_edd_purchase', [ $payment_id ] );
    }
    ...
    public static function send_purchase_async( int $payment_id ) {
        ...
        $event_id = Ratul_ACT_Dedup::get_event_id( $payment_id );
        ...
        if ( get_option( 'ratul_act_meta_enabled', 0 ) ) {
            if ( ! Ratul_ACT_Dedup::was_sent( $payment_id, 'meta' ) ) {
                if ( Ratul_ACT_Consent::is_granted( 'meta' ) ) {
                    $e = ( new Ratul_ACT_Event( 'Purchase', $event_id ) )
                        ->set_user_data( $user_data )
                        ->set_custom_data( $custom_data );
                    $send_result = Ratul_ACT_Meta::send( $e );
                    if ( ( $send_result['status'] ?? '' ) === 'success' ) {
                        Ratul_ACT_Dedup::mark_as_sent( $payment_id, 'meta' );
                    } else {
                        Ratul_ACT_Retry::maybe_queue( 'meta', $send_result, Ratul_ACT_Retry::event_to_args( $e ) );
                    }
                } else {
                    Ratul_ACT_Logger::log( 'skipped', 'meta', 'Consent not granted', '', $event_id, $payment_id, 'Purchase' );
                }
            } else {
                Ratul_ACT_Logger::log( 'dedup_blocked', 'meta', 'Already sent', '', $event_id, $payment_id, 'Purchase' );
            }
        }

        // ── Google ───────────────────────────────────────────────────────────
        if ( get_option( 'ratul_act_google_enabled', 0 ) ) {
            if ( ! Ratul_ACT_Dedup::was_sent( $payment_id, 'google' ) ) {
                if ( Ratul_ACT_Consent::is_granted( 'google' ) ) {
                    $e = ( new Ratul_ACT_Event( 'Purchase', $event_id ) )
                        ->set_user_data( $user_data )
                        ->set_custom_data( $custom_data );
                    $send_result = Ratul_ACT_Google::send( $e );
                    if ( ( $send_result['status'] ?? '' ) === 'success' ) {
                        Ratul_ACT_Dedup::mark_as_sent( $payment_id, 'google' );
                    } else {
                        Ratul_ACT_Retry::maybe_queue( 'google', $send_result, Ratul_ACT_Retry::event_to_args( $e ) );
                    }
                } else {
                    Ratul_ACT_Logger::log( 'skipped', 'google', 'Consent not granted', '', $event_id, $payment_id, 'Purchase' );
                }
            } else {
                Ratul_ACT_Logger::log( 'dedup_blocked', 'google', 'Already sent', '', $event_id, $payment_id, 'Purchase' );
            }
        }

        // ── TikTok ───────────────────────────────────────────────────────────
        if ( get_option( 'ratul_act_tiktok_enabled', 0 ) ) {
            if ( ! Ratul_ACT_Dedup::was_sent( $payment_id, 'tiktok' ) ) {
                if ( Ratul_ACT_Consent::is_granted( 'tiktok' ) ) {
                    $e = ( new Ratul_ACT_Event( 'Purchase', $event_id ) )
                        ->set_user_data( $user_data )
                        ->set_custom_data( $custom_data );
                    $send_result = Ratul_ACT_TikTok::send( $e );
                    if ( ( $send_result['status'] ?? '' ) === 'success' ) {
                        Ratul_ACT_Dedup::mark_as_sent( $payment_id, 'tiktok' );
                    } else {
                        Ratul_ACT_Retry::maybe_queue( 'tiktok', $send_result, Ratul_ACT_Retry::event_to_args( $e ) );
                    }
                } else {
                    Ratul_ACT_Logger::log( 'skipped', 'tiktok', 'Consent not granted', '', $event_id, $payment_id, 'Purchase' );
                }
            } else {
                Ratul_ACT_Logger::log( 'dedup_blocked', 'tiktok', 'Already sent', '', $event_id, $payment_id, 'Purchase' );
            }
        }
    }
    ...
    private static function build_data_edd3( int $payment_id ): ?array {
        ...
        if ( ! empty( $order->email ) ) {
            $user_data['email'] = Ratul_ACT_Hasher::hash_email( $order->email );
        }
        if ( ! empty( $order->ip ) ) {
        ...
        if ( $customer ) {
            ...
            if ( ! empty( $name_parts[0] ) ) {
                $user_data['first_name'] = Ratul_ACT_Hasher::hash_name( $name_parts[0] );
            }
            if ( ! empty( $name_parts[1] ) ) {
                $user_data['last_name'] = Ratul_ACT_Hasher::hash_name( $name_parts[1] );
            }
        }
    ...
    private static function build_data_legacy( int $payment_id ): ?array {
        ...
        if ( ! empty( $payment->email ) ) {
            $user_data['email'] = Ratul_ACT_Hasher::hash_email( $payment->email );
        }
        if ( ! empty( $user_info['first_name'] ) ) {
            $user_data['first_name'] = Ratul_ACT_Hasher::hash_name( $user_info['first_name'] );
        }
        if ( ! empty( $user_info['last_name'] ) ) {
            $user_data['last_name'] = Ratul_ACT_Hasher::hash_name( $user_info['last_name'] );
        }
        if ( ! empty( $payment->ip ) ) {
    ...
    public static function on_registration( string $user_login, string $user_email, int $user_id ) {
        ...
        $event_id  = Ratul_ACT_Dedup::generate_event_id( 'edd_lead_' . $user_id . '_' . wp_generate_uuid4() );
        $user_data = [];

        if ( ! empty( $user_email ) ) {
            $user_data['email'] = Ratul_ACT_Hasher::hash_email( $user_email );
        }
        ...
        $event = new Ratul_ACT_Event( 'Lead', $event_id );
        $event->set_user_data( $user_data );
        $event->set_custom_data( [ 'currency' => 'USD', 'value' => 0.0, 'contents' => [] ] );

        if ( $meta_on && Ratul_ACT_Consent::is_granted( 'meta' ) ) {
            $result = Ratul_ACT_Meta::send( $event );
            Ratul_ACT_Retry::maybe_queue( 'meta', $result, Ratul_ACT_Retry::event_to_args( $event ) );
        }
        if ( $tiktok_on && Ratul_ACT_Consent::is_granted( 'tiktok' ) ) {
            $result = Ratul_ACT_TikTok::send( $event );
            Ratul_ACT_Retry::maybe_queue( 'tiktok', $result, Ratul_ACT_Retry::event_to_args( $event ) );
        }
    }
}
