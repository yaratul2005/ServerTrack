 ...
 *      the ratul_act_pending_abandonments option keyed by session hash.
 *      The record contains the cart snapshot, user_data, and the timestamp
 *      of the last update.
 *
 *   2. A WP-Cron job (ratul_act_check_abandonments) runs every 15 minutes.
 ...
 *   `ratul_act_abandonment_window` filter (value in minutes).
 *
 * Options:
 *   ratul_act_source_abandonment_enabled  — 0/1, default 0 (opt-in)
 *   ratul_act_abandonment_window_minutes  — int, default 60
 *
 * Changes in v1.1:
 *   NEW-4 (CRITICAL): fire_abandonment_event() now checks
 *     Ratul_ACT_Consent::is_granted() for each platform before sending.
 ...
 *     same 4-tier priority chain used in Ratul_ACT_Frontend v2.2:
 ...
 */
class Ratul_ACT_WooAbandonment {

    /** Option key for the pending abandonment records store. */
    const STORE_KEY = 'ratul_act_pending_abandonments';

    /** Cron hook name. */
    const CRON_HOOK = 'ratul_act_check_abandonments';

    public static function init() {
        if ( ! get_option( 'ratul_act_enabled', 1 ) ) return;
        if ( ! get_option( 'ratul_act_source_abandonment_enabled', 0 ) ) return;
        ...
        if ( ! wp_next_scheduled( self::CRON_HOOK ) ) {
            wp_schedule_event( time(), 'ratul_act_15min', self::CRON_HOOK );
        }
    ...
    public static function add_cron_interval( array $schedules ): array {
        if ( ! isset( $schedules['ratul_act_15min'] ) ) {
            $schedules['ratul_act_15min'] = [
                'interval' => 15 * MINUTE_IN_SECONDS,
                'display'  => __( 'Every 15 minutes (Ratul_ACT)', 'ratul-ads-conversion-tracker' ),
            ];
        }
        return $schedules;
    }
    ...
    public static function on_cart_updated() {
        ...
        $event_id = ! empty( $existing['event_id'] )
            ? $existing['event_id']
            : Ratul_ACT_Dedup::generate_event_id( 'abandon_' . $session_id );
    ...
    public static function process_abandonments() {
         ...
         * Default: 60. Override via `ratul_act_abandonment_window` filter.
         *
         * @param int $window_minutes
         */
        $window_minutes = (int) apply_filters(
            'ratul_act_abandonment_window',
            (int) get_option( 'ratul_act_abandonment_window_minutes', 60 )
        );
        ...
        $meta_on   = (bool) get_option( 'ratul_act_meta_enabled', 0 );
        $tiktok_on = (bool) get_option( 'ratul_act_tiktok_enabled', 0 );
        $google_on = (bool) get_option( 'ratul_act_google_enabled', 0 );
    ...
    private static function fire_abandonment_event(
        ...
        $event_id  = $record['event_id']  ?? Ratul_ACT_Dedup::generate_event_id();
        ...
        if ( $meta_on && Ratul_ACT_Consent::is_granted( 'meta' ) ) {
            $e = ( new Ratul_ACT_Event( 'InitiateCheckout', $event_id ) )
        ...
        // ── GOOGLE: begin_checkout ───────────────────────────────────────────
        // NEW-4 fix: consent check added — was missing in v1.0.
        if ( $google_on && Ratul_ACT_Consent::is_granted( 'google' ) ) {
            $e = ( new Ratul_ACT_Event( 'begin_checkout', $event_id ) )
                ->set_user_data( $user_data )
                ->set_custom_data( $custom_data );
     ...
     * priority chain used in Ratul_ACT_Frontend v2.2:
    ...
    private static function build_session_user_data(): array {
        ...
        // NEW-5 fix: 4-tier IP chain — mirrors Ratul_ACT_Frontend::get_request_ip().
        $ip = '';
        ...
        if ( is_user_logged_in() ) {
            ...
            if ( $user->user_email ) {
                $data['email']       = Ratul_ACT_Hasher::hash_email( $user->user_email );
                $data['external_id'] = Ratul_ACT_Hasher::hash( (string) $user->ID );
            }
        }
