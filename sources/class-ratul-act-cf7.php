 ...
 *   2. send() results are now checked and routed to Ratul_ACT_Retry::maybe_queue()
 ...
 *
 *   on_form_sent() called Ratul_ACT_Hasher::hash_phone( $phone ) with no
 ...
 *   build_order_user_data() in Ratul_ACT_WooCommerce.
 *
 *   Admin can also override per-form via ratul_act_cf7_mappings[$form_id]['country'].
...
class Ratul_ACT_CF7 {
     ...
     * Mirrors the static table in Ratul_ACT_WooCommerce::build_order_user_data().
     */
    ...
    public static function init() {
        if ( ! get_option( 'ratul_act_source_cf7_enabled', 0 ) ) return;
    ...
    public static function on_form_sent( WPCF7_ContactForm $contact_form ) {
        ...
        $event_id = Ratul_ACT_Dedup::generate_event_id( 'lead_cf7_' . $form_id . '_' . wp_generate_uuid4() );
        ...
        $mappings = get_option( 'ratul_act_cf7_mappings', [] );
        ...
        if ( ! empty( $email ) ) {
            $user_data['email'] = Ratul_ACT_Hasher::hash_email( sanitize_email( $email ) );
        }
        ...
        if ( ! empty( $phone ) ) {
            ...
            $user_data['phone'] = Ratul_ACT_Hasher::hash_phone( sanitize_text_field( $phone ), $cc );
        }

        $name = $posted_data[ $name_field ] ?? '';
        if ( ! empty( $name ) ) {
            $parts = explode( ' ', trim( sanitize_text_field( $name ) ), 2 );
            $user_data['first_name'] = Ratul_ACT_Hasher::hash_name( $parts[0] );
            if ( ! empty( $parts[1] ) ) {
                $user_data['last_name'] = Ratul_ACT_Hasher::hash_name( $parts[1] );
            }
        }
