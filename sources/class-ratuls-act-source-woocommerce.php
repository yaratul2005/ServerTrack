...
/**
 * Ratuls_ACT_Source_WooCommerce  v3.3.2
 *
 ...
 * settings page (ratuls-act-sources).
 *
 * Changelog
 * ----------
 ...
 */
class Ratuls_ACT_Source_WooCommerce {
    ...
    public static function init(): void {
        ...
        if ( self::opt( 'ratuls_act_source_woo_enabled', 1 ) ) {
            self::register_core_hooks();
        }

        if ( self::opt( 'ratuls_act_source_order_status_enabled', 1 ) ) {
            add_action( 'woocommerce_order_status_changed', [ self::class, 'handle_order_status_change' ], 10, 4 );
        }

        if ( self::opt( 'ratuls_act_source_wishlist_enabled', 0 ) ) {
        ...
        if ( self::opt( 'ratuls_act_source_partial_refund_enabled', 1 ) ) {
    ...
    private static function register_core_hooks(): void {
        // Commented out to prevent duplicate CAPI sends (handled by Ratuls_ACT_WooCommerce core triggers)
        ...
        add_action( 'wp', [ 'Ratuls_ACT_Consent', 'capture_snapshot' ] );
        add_action( 'woocommerce_checkout_order_processed', [ 'Ratuls_ACT_Consent', 'capture_for_order' ],  9, 1 );
    ...
    public static function handle_order_status_change(
        ...
        $user_data   = [ 'external_id' => Ratuls_ACT_Identity::get_external_id_for_order( $order ) ];
        $custom_data = [
            'order_id'     => $order_id,
        ...
        $event_id = Ratuls_ACT_Hasher::event_id( $event_name, $order_id . '_' . $new_status );
        $event    = ( new Ratuls_ACT_Event( $event_name, $event_id ) )
            ->set_user_data( $user_data )
            ->set_custom_data( $custom_data );

        Ratuls_ACT_Core::dispatch_to_all( $event, $dedup_key );
    }
    ...
    private static function fire_add_to_wishlist_event( int $product_id, string $source ): void {
        ...
        $pending_platforms = array_filter(
            $all_platforms,
            static fn( $p ) => ! Ratuls_ACT_Dedup::already_sent( $dedup_key, $p )
        );
        ...
        $user_data   = [ 'external_id' => Ratuls_ACT_Identity::get_external_id_for_user( get_current_user_id() ) ];
        $custom_data = [
        ...
        $event_id = Ratuls_ACT_Hasher::event_id( 'AddToWishlist', $uid_part . '_' . $product_id );
        $event    = ( new Ratuls_ACT_Event( 'AddToWishlist', $event_id ) )
            ->set_user_data( $user_data )
            ->set_custom_data( $custom_data );

        // Dispatch only to pending platforms (Meta + TikTok, minus already-sent)
        Ratuls_ACT_Core::dispatch_to_platforms( $event, array_values( $pending_platforms ), $dedup_key );
    }
    ...
    public static function handle_partial_refund( int $order_id, int $refund_id ): void {
        ...
        $event_id = Ratuls_ACT_Hasher::event_id( 'Purchase', 'partial_refund_' . $refund_id );
        $event    = ( new Ratuls_ACT_Event( 'Purchase', $event_id ) )
            ->set_user_data( $user_data )
            ->set_custom_data( $custom_data );
    ...
    public static function fire_manual_purchase( int $order_id ): array {
        ...
        if ( Ratuls_ACT_Dedup::already_sent( $event_id, 'meta' )
          && Ratuls_ACT_Dedup::already_sent( $event_id, 'tiktok' )
          && Ratuls_ACT_Dedup::already_sent( $event_id, 'google' ) ) {
            return [ 'success' => false, 'message' => 'Manual Purchase event has already been sent.' ];
        }

        $user_data   = [ 'external_id' => Ratuls_ACT_Identity::get_external_id_for_order( $order ) ];
        $custom_data = Ratuls_ACT_Catalog::from_order( $order ) ?: [];

        $event       = ( new Ratuls_ACT_Event( 'Purchase', $event_id ) )
            ->set_user_data( $user_data )
            ->set_custom_data( $custom_data );

        Ratuls_ACT_Core::dispatch_to_all( $event, $event_id );

        // Mark the order so we know it was manually verified and sent
        $order->update_meta_data( '_ratuls_act_manual_purchase_sent', 'yes' );
        $order->save();
    ...
    public static function handle_purchase( int $order_id ): void {
        $order = wc_get_order( $order_id );
        if ( ! $order ) return;
        if ( Ratuls_ACT_Dedup::already_sent( $order_id, 'meta' )
          && Ratuls_ACT_Dedup::already_sent( $order_id, 'tiktok' )
          && Ratuls_ACT_Dedup::already_sent( $order_id, 'google' ) ) {
            return;
        }
        $user_data   = [ 'external_id' => Ratuls_ACT_Identity::get_external_id_for_order( $order ) ];
        $custom_data = Ratuls_ACT_Catalog::from_order( $order ) ?: [];
        ...
        $event       = ( new Ratuls_ACT_Event( 'Purchase', $event_id ) )
            ->set_user_data( $user_data )
            ->set_custom_data( $custom_data );
        Ratuls_ACT_Core::dispatch_to_all( $event, $order_id );
    }
    ...
    public static function handle_view_content( int $order_id ): void {
        ...
        if ( ! $order ) return;
        $user_data   = [ 'external_id' => Ratuls_ACT_Identity::get_external_id_for_order( $order ) ];
        $custom_data = Ratuls_ACT_Catalog::from_order_summary( $order ) ?: [];
        global $ratuls_act_page_load_id;
        if ( empty( $ratuls_act_page_load_id ) ) {
            $ratuls_act_page_load_id = wp_generate_uuid4();
        }
        $event_id    = Ratuls_ACT_Hasher::event_id( 'ViewContent', $order_id . '_' . $ratuls_act_page_load_id );
        $event       = ( new Ratuls_ACT_Event( 'ViewContent', $event_id ) )
            ->set_user_data( $user_data )
            ->set_custom_data( $custom_data );
        Ratuls_ACT_Core::dispatch_to_all( $event );
    }
    ...
    public static function handle_add_to_cart(
        ...
        $event_id = Ratuls_ACT_Hasher::event_id( 'AddToCart', $cart_item_key . '_' . wp_generate_uuid4() );
        $event    = ( new Ratuls_ACT_Event( 'AddToCart', $event_id ) )
            ->set_user_data( $user_data )
            ->set_custom_data( $custom_data );
        Ratuls_ACT_Core::dispatch_to_all( $event );
    }
    ...
    public static function handle_initiate_checkout(): void {
        ...
        $user_data   = [ 'external_id' => Ratuls_ACT_Identity::get_external_id_for_user( get_current_user_id() ) ];
        $custom_data = Ratuls_ACT_Catalog::from_cart();
        $event_id    = Ratuls_ACT_Hasher::event_id( 'InitiateCheckout', $session_key );
    ...
    public static function handle_add_payment_info( int $order_id ): void {
        ...
        $event_id    = Ratuls_ACT_Hasher::event_id( 'AddPaymentInfo', $order_id . '_' . $ratuls_act_page_load_id );
        $event       = ( new Ratuls_ACT_Event( 'AddPaymentInfo', $event_id ) )
            ->set_user_data( $user_data )
            ->set_custom_data( $custom_data );
        Ratuls_ACT_Core::dispatch_to_all( $event );
    }
    ...
    public static function handle_complete_registration( int $customer_id ): void {
        ...
        $event_id  = Ratuls_ACT_Hasher::event_id( 'CompleteRegistration', $customer_id );
        $event     = ( new Ratuls_ACT_Event( 'CompleteRegistration', $event_id ) )
            ->set_user_data( $user_data )
    ...
    public static function handle_full_refund( int $order_id, int $refund_id ): void {
        ...
        if ( Ratuls_ACT_Dedup::already_sent( $dedup_key, 'meta' )
        ...
        $event_id = Ratuls_ACT_Hasher::event_id( 'Purchase', 'full_refund_' . $order_id );
        $event    = ( new Ratuls_ACT_Event( 'Purchase', $event_id ) )
            ->set_user_data( $user_data )
            ->set_custom_data( $custom_data );
        Ratuls_ACT_Core::dispatch_to_all( $event, $dedup_key );
