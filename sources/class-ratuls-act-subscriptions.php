 ...
 * (Ratuls_ACT_Dedup::already_sent / mark_string_sent / get / set) so that
 ...
 *           get_event_id($key)     → Ratuls_ACT_Dedup::get( $key )
 *           store_event_id($key)   → Ratuls_ACT_Dedup::set_event( $key, $id )
 *           was_sent($key, $p)     → Ratuls_ACT_Dedup::already_sent( $key, $p )
 *           mark_as_sent($key, $p) → Ratuls_ACT_Dedup::mark_string_sent( $key, $p )
 ...
 */
class Ratuls_ACT_Subscriptions {
    ...
    public static function init(): void {
        ...
        // Async cron handlers
        add_action( 'ratuls_act_send_sub_renewal',   [ self::class, 'send_renewal_async' ],    10, 2 );
        add_action( 'ratuls_act_send_sub_cancelled', [ self::class, 'send_cancelled_async' ],  10, 1 );
        add_action( 'ratuls_act_send_sub_paused',    [ self::class, 'send_paused_async' ],     10, 1 );
    }
    ...
    public static function on_renewal( WC_Subscription $subscription, WC_Order $renewal_order ): void {
        ...
        wp_schedule_single_event( time(), 'ratuls_act_send_sub_renewal', [ $sub_id, $ord_id ] );
        spawn_cron();
    }

    public static function on_cancelled( WC_Subscription $subscription ): void {
        if ( ! get_option( 'ratuls_act_enabled', 1 ) ) return;
        wp_schedule_single_event( time(), 'ratuls_act_send_sub_cancelled', [ $subscription->get_id() ] );
        spawn_cron();
    }

    public static function on_paused( WC_Subscription $subscription ): void {
        if ( ! get_option( 'ratuls_act_enabled', 1 ) ) return;
        if ( ! get_option( 'ratuls_act_meta_enabled', 0 ) ) return;
        wp_schedule_single_event( time(), 'ratuls_act_send_sub_paused', [ $subscription->get_id() ] );
        spawn_cron();
    }
    ...
    public static function send_renewal_async( int $sub_id, int $order_id ): void {
        ...
        if ( ! $order ) {
            Ratuls_ACT_Logger::log( 'error', 'all', 'Renewal: order #' . $order_id . ' not found.', '', '', $order_id, 'Renewal' );
            return;
        }
        ...
        if ( empty( $event_id ) ) {
            $event_id = Ratuls_ACT_Dedup::generate_event_id( $dedup_key );
        ...
        if ( get_option( 'ratuls_act_meta_enabled', 0 )
            && ! Ratuls_ACT_Dedup::already_sent( $dedup_key, 'meta' )
            && Ratuls_ACT_Consent::is_granted( 'meta', $order_id ) ) {
            $e      = ( new Ratuls_ACT_Event( 'Purchase', $event_id ) )->set_user_data( $user_data )->set_custom_data( $custom_data );
            $result = Ratuls_ACT_Meta::send( $e );
            if ( ( $result['status'] ?? '' ) === 'success' ) {
                Ratuls_ACT_Dedup::mark_string_sent( $dedup_key, 'meta' );
            } else {
                Ratuls_ACT_Retry::maybe_queue( 'meta', $result, Ratuls_ACT_Retry::event_to_args( $e ) );
            }
            Ratuls_ACT_Logger::log( $result['status'] ?? 'error', 'meta', 'Subscription renewal #' . $sub_id, '', $event_id, $order_id, 'Renewal' );
        }

        // TIKTOK
        if ( get_option( 'ratuls_act_tiktok_enabled', 0 )
            && ! Ratuls_ACT_Dedup::already_sent( $dedup_key, 'tiktok' )
            && Ratuls_ACT_Consent::is_granted( 'tiktok', $order_id ) ) {
            $e      = ( new Ratuls_ACT_Event( 'Purchase', $event_id ) )->set_user_data( $user_data )->set_custom_data( $custom_data );
            $result = Ratuls_ACT_TikTok::send( $e );
            if ( ( $result['status'] ?? '' ) === 'success' ) {
                Ratuls_ACT_Dedup::mark_string_sent( $dedup_key, 'tiktok' );
            } else {
                Ratuls_ACT_Retry::maybe_queue( 'tiktok', $result, Ratuls_ACT_Retry::event_to_args( $e ) );
            }
            Ratuls_ACT_Logger::log( $result['status'] ?? 'error', 'tiktok', 'Subscription renewal #' . $sub_id, '', $event_id, $order_id, 'Renewal' );
        }

        // GOOGLE
        if ( get_option( 'ratuls_act_google_enabled', 0 )
            && ! Ratuls_ACT_Dedup::already_sent( $dedup_key, 'google' )
            && Ratuls_ACT_Consent::is_granted( 'google', $order_id ) ) {
            $e      = ( new Ratuls_ACT_Event( 'Purchase', $event_id ) )->set_user_data( $user_data )->set_custom_data( $custom_data );
            $result = Ratuls_ACT_Google::send( $e );
            if ( ( $result['status'] ?? '' ) === 'success' ) {
                Ratuls_ACT_Dedup::mark_string_sent( $dedup_key, 'google' );
            } else {
                Ratuls_ACT_Retry::maybe_queue( 'google', $result, Ratuls_ACT_Retry::event_to_args( $e ) );
            }
            Ratuls_ACT_Logger::log( $result['status'] ?? 'error', 'google', 'Subscription renewal #' . $sub_id, '', $event_id, $order_id, 'Renewal' );
        }
    }
    ...
    public static function send_cancelled_async( int $sub_id ): void {
        ...
        if ( get_option( 'ratuls_act_meta_enabled', 0 )
            && ! Ratuls_ACT_Dedup::already_sent( $dedup_key, 'meta' ) ) {
            $e      = ( new Ratuls_ACT_Event( 'SubscriptionCancelled', $event_id ) )->set_user_data( $user_data )->set_custom_data( $custom );
            $result = Ratuls_ACT_Meta::send( $e );
            if ( ( $result['status'] ?? '' ) === 'success' ) Ratuls_ACT_Dedup::mark_string_sent( $dedup_key, 'meta' );
            Ratuls_ACT_Logger::log( $result['status'] ?? 'error', 'meta', 'Sub cancelled #' . $sub_id, '', $event_id, $order_id, 'SubscriptionCancelled' );
        }
        ...
        if ( get_option( 'ratuls_act_tiktok_enabled', 0 )
            ...
            $e      = ( new Ratuls_ACT_Event( 'PlaceAnOrder', $event_id ) )->set_user_data( $user_data )->set_custom_data( $neg );
            $result = Ratuls_ACT_TikTok::send( $e );
            if ( ( $result['status'] ?? '' ) === 'success' ) Ratuls_ACT_Dedup::mark_string_sent( $dedup_key, 'tiktok' );
            Ratuls_ACT_Logger::log( $result['status'] ?? 'error', 'tiktok', 'Sub cancelled #' . $sub_id, '', $event_id, $order_id, 'SubscriptionCancelled' );
        }
        ...
        if ( get_option( 'ratuls_act_google_enabled', 0 )
            && ! Ratuls_ACT_Dedup::already_sent( $dedup_key, 'google' ) ) {
            $neg    = array_merge( $custom, [ 'value' => -1 * abs( $custom['value'] ) ] );
            $e      = ( new Ratuls_ACT_Event( 'refund', $event_id ) )->set_user_data( $user_data )->set_custom_data( $neg );
            $result = Ratuls_ACT_Google::send( $e );
            if ( ( $result['status'] ?? '' ) === 'success' ) Ratuls_ACT_Dedup::mark_string_sent( $dedup_key, 'google' );
            Ratuls_ACT_Logger::log( $result['status'] ?? 'error', 'google', 'Sub cancelled #' . $sub_id, '', $event_id, $order_id, 'SubscriptionCancelled' );
        }
    }
    ...
    public static function send_paused_async( int $sub_id ): void {
        ...
        if ( Ratuls_ACT_Dedup::already_sent( $dedup_key, 'meta' ) ) return;
        ...
        $e      = ( new Ratuls_ACT_Event( 'SubscriptionPaused', $event_id ) )->set_user_data( $user_data )->set_custom_data( $custom );
        $result = Ratuls_ACT_Meta::send( $e );
        if ( ( $result['status'] ?? '' ) === 'success' ) Ratuls_ACT_Dedup::mark_string_sent( $dedup_key, 'meta' );
        Ratuls_ACT_Logger::log( $result['status'] ?? 'error', 'meta', 'Sub paused #' . $sub_id, '', $event_id, 0, 'SubscriptionPaused' );
    }
    ...
    private static function get_string_event_id( string $key ): string {
        $stored = get_option(
            Ratuls_ACT_Dedup::OPTIONS_PREFIX . sanitize_key( $key . '_event_id' ),
            ''
        );
    ...
    private static function build_order_user_data( WC_Abstract_Order $order ): array {
        ...
        if ( $phone ) $data['phone'] = Ratuls_ACT_Hasher::hash_phone( $phone, '' );
        foreach ( [ 'first_name', 'last_name', 'city', 'state', 'zip', 'country' ] as $field ) {
            ...
            if ( $val ) $data[$field] = Ratuls_ACT_Hasher::hash( $val );
        }
        $customer_id       = (int) ( method_exists( $order, 'get_customer_id' ) ? $order->get_customer_id() : 0 );
        $data['external_id'] = Ratuls_ACT_Identity::get_external_id_for_order( $order );
        return $data;
    }
