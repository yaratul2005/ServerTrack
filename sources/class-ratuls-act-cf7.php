 ...
 *   2. send() results are now checked and routed to Ratuls_ACT_Retry::maybe_queue()
 ...
 *
 *   on_form_sent() called Ratuls_ACT_Hasher::hash_phone( $phone ) with no
 ...
 *   build_order_user_data() in Ratuls_ACT_WooCommerce.
 *
 *   Admin can also override per-form via ratuls_act_cf7_mappings[$form_id]['country'].
 *   If no country is resolvable, passes '' (safe — hash_phone degrades gracefully).
 */
class Ratuls_ACT_CF7 {

    /**
     * E.164 dialling codes keyed by ISO 3166-1 alpha-2 country code.
     * Mirrors the static table in Ratuls_ACT_WooCommerce::build_order_user_data().
     */
    ...
    public static function init() {
        if ( ! get_option( 'ratuls_act_source_cf7_enabled', 0 ) ) return;
    ...
    public static function on_form_sent( WPCF7_ContactForm $contact_form ) {
        ...
        $event_id = Ratuls_ACT_Dedup::generate_event_id( 'lead_cf7_' . $form_id . '_' . wp_generate_uuid4() );
        ...
        $mappings = get_option( 'ratuls_act_cf7_mappings', [] );
        ...
        if ( ! empty( $email ) ) {
            $user_data['email'] = Ratuls_ACT_Hasher::hash_email( sanitize_email( $email ) );
        }
        ...
        if ( ! empty( $phone ) ) {
            ...
            $user_data['phone'] = Ratuls_ACT_Hasher::hash_phone( sanitize_text_field( $phone ), $cc );
        }

        $name = $posted_data[ $name_field ] ?? '';
        if ( ! empty( $name ) ) {
            $parts = explode( ' ', trim( sanitize_text_field( $name ) ), 2 );
            $user_data['first_name'] = Ratuls_ACT_Hasher::hash_name( $parts[0] );
            if ( ! empty( $parts[1] ) ) {
                $user_data['last_name'] = Ratuls_ACT_Hasher::hash_name( $parts[1] );
            }
        }
