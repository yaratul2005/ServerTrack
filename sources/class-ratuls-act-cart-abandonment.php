...
/**
 * Ratuls_ACT_CartAbandonment  v1.1
 *
 ...
 *           Google block now fires when ratuls_act_google_enabled is set and
 ...
 *   Filter: ratuls_act_abandonment_window_seconds (default: 3600)
 */
class Ratuls_ACT_CartAbandonment {

    const TRANSIENT_PREFIX_PENDING   = 'ratuls_act_cart_pending_';
    const TRANSIENT_PREFIX_COMPLETED = 'ratuls_act_cart_done_';
    ...
    public static function init(): void {
        ...
        add_action( 'ratuls_act_check_abandonment',    [ self::class, 'check_abandonment' ],    10, 1 );
    }
    ...
    public static function on_add_to_cart(
        ...
        if ( ! get_option( 'ratuls_act_meta_enabled', 0 )
            && ! get_option( 'ratuls_act_tiktok_enabled', 0 )
            && ! get_option( 'ratuls_act_google_enabled', 0 ) ) return;

        $session_id = self::get_session_id();
        if ( ! $session_id ) return;

        $window    = (int) apply_filters( 'ratuls_act_abandonment_window_seconds', self::DEFAULT_WINDOW );
        ...
        if ( ! wp_next_scheduled( 'ratuls_act_check_abandonment', [ $session_id ] ) ) {
            wp_schedule_single_event( time() + $window, 'ratuls_act_check_abandonment', [ $session_id ] );
        }
    }
    ...
    public static function check_abandonment( string $session_id ): void {
        ...
        if ( get_transient( $done_key ) ) {
            Ratuls_ACT_Logger::log( 'skipped', 'all', 'Cart abandonment: session completed purchase.', '', '', 0, 'CartAbandonment' );
            return;
        }
        ...
        if ( ! $snapshot ) {
            Ratuls_ACT_Logger::log( 'skipped', 'all', 'Cart abandonment: no snapshot found for session.', '', '', 0, 'CartAbandonment' );
            return;
        }
        ...
        if ( ! $email ) {
            Ratuls_ACT_Logger::log( 'skipped', 'all', 'Cart abandonment: no email — anonymous session, skipping.', '', '', 0, 'CartAbandonment' );
            return;
        }

        // Build minimal user_data
        $user_data = [
            'email' => Ratuls_ACT_Hasher::hash_email( $email ),
        ];
        ...
        $event_id    = Ratuls_ACT_Dedup::generate_event_id( 'abandon_' . $session_id );
        $custom_data = [
        ...
        $google_on = get_option( 'ratuls_act_google_enabled', 0 );

        if ( $meta_on && Ratuls_ACT_Consent::is_granted( 'meta' ) ) {
            $e      = ( new Ratuls_ACT_Event( 'InitiateCheckout', $event_id ) )->set_user_data( $user_data )->set_custom_data( $custom_data );
            $result = Ratuls_ACT_Meta::send( $e );
            Ratuls_ACT_Logger::log( $result['status'] ?? 'error', 'meta', 'Cart abandonment email=' . $email, '', $event_id, 0, 'CartAbandonment' );
        }

        if ( $tiktok_on && Ratuls_ACT_Consent::is_granted( 'tiktok' ) ) {
            $e      = ( new Ratuls_ACT_Event( 'InitiateCheckout', $event_id ) )->set_user_data( $user_data )->set_custom_data( $custom_data );
            $result = Ratuls_ACT_TikTok::send( $e );
            Ratuls_ACT_Logger::log( $result['status'] ?? 'error', 'tiktok', 'Cart abandonment email=' . $email, '', $event_id, 0, 'CartAbandonment' );
        }

        // BUG-M3 fix: Google Ads abandonment block — was entirely missing
        if ( $google_on && Ratuls_ACT_Consent::is_granted( 'google' ) ) {
            $e      = ( new Ratuls_ACT_Event( 'InitiateCheckout', $event_id ) )->set_user_data( $user_data )->set_custom_data( $custom_data );
            $result = Ratuls_ACT_Google::send( $e );
            Ratuls_ACT_Logger::log( $result['status'] ?? 'error', 'google', 'Cart abandonment email=' . $email, '', $event_id, 0, 'CartAbandonment' );
        }
