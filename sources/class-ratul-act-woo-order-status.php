 ...
 * @since   5.0.0
 */
class Ratul_ACT_WooOrderStatus {
    ...
    public static function init(): void {
        ...
        add_action(
            'ratul_act_send_order_status_event',
    ...
    public static function on_order_status_change( int $order_id, WC_Order $order ): void {
        ...
        $meta_sent   = ! get_option( 'ratul_act_meta_enabled', 0 )   || Ratul_ACT_Dedup::exists( $base_key . '_meta' );
        $tiktok_sent = ! get_option( 'ratul_act_tiktok_enabled', 0 ) || Ratul_ACT_Dedup::exists( $base_key . '_tiktok' );
        $google_sent = ! get_option( 'ratul_act_google_enabled', 0 ) || Ratul_ACT_Dedup::exists( $base_key . '_google' );
    ...
    public static function send_status_event_async( int $order_id, string $status ): void {
        ...
        if ( get_option( 'ratul_act_meta_enabled', 0 )
            ...
            $e = ( new Ratul_ACT_Event( self::META_EVENT, $event_id ) )
            ...
            Ratul_ACT_Logger::log( $r['status'] ?? 'error', 'meta', $event_label . ' #' . $order_id, '', $event_id, $order_id, $event_label, $emq );
        }
        ...
        if ( get_option( 'ratul_act_tiktok_enabled', 0 )
            ...
            $e = ( new Ratul_ACT_Event( $tiktok_event, $event_id ) )
                ->set_user_data( $user_data )
                ->set_custom_data( $custom_data );
            ...
            Ratul_ACT_Logger::log( $r['status'] ?? 'error', 'tiktok', $event_label . ' #' . $order_id, '', $event_id, $order_id, $event_label, $emq );
        }
        ...
        if ( get_option( 'ratul_act_google_enabled', 0 )
            ...
            $e = ( new Ratul_ACT_Event( $google_event, $event_id ) )
                ->set_user_data( $user_data )
                ->set_custom_data( $custom_data );
            ...
            Ratul_ACT_Logger::log( $r['status'] ?? 'error', 'google', $event_label . ' #' . $order_id, '', $event_id, $order_id, $event_label, $emq );
        }
    }
