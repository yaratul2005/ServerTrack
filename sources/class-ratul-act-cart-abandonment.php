...
/**
 * Ratul_ACT_CartAbandonment  v1.1
 *
 ...
 *           Google block now fires when ratul_act_google_enabled is set and
 ...
 *   Filter: ratul_act_abandonment_window_seconds (default: 3600)
 */
class Ratul_ACT_CartAbandonment {

    const TRANSIENT_PREFIX_PENDING   = 'ratul_act_cart_pending_';
    const TRANSIENT_PREFIX_COMPLETED = 'ratul_act_cart_done_';
    ...
    public static function init(): void {
        ...
        add_action( 'ratul_act_check_abandonment',    [ self::class, 'check_abandonment' ],    10, 1 );
    }
    ...
    public static function on_add_to_cart(
        ...
        if ( ! get_option( 'ratul_act_meta_enabled', 0 )
            && ! get_option( 'ratul_act_tiktok_enabled', 0 )
            && ! get_option( 'ratul_act_google_enabled', 0 ) ) return;

        $session_id = self::get_session_id();
        if ( ! $session_id ) return;

        $window    = (int) apply_filters( 'ratul_act_abandonment_window_seconds', self::DEFAULT_WINDOW );
        ...
        if ( ! wp_next_scheduled( 'ratul_act_check_abandonment', [ $session_id ] ) ) {
            wp_schedule_single_event( time() + $window, 'ratul_act_check_abandonment', [ $session_id ] );
        }
    }
    ...
    public static function check_abandonment( string $session_id ): void {
        ...
        if ( get_transient( $done_key ) ) {
            Ratul_ACT_Logger::log( 'skipped', 'all', 'Cart abandonment: session completed purchase.', '', '', 0, 'CartAbandonment' );
            return;
        }
        ...
        if ( ! $snapshot ) {
            Ratul_ACT_Logger::log( 'skipped', 'all', 'Cart abandonment: no snapshot found for session.', '', '', 0, 'CartAbandonment' );
            return;
        }
        ...
        if ( ! $email ) {
            Ratul_ACT_Logger::log( 'skipped', 'all', 'Cart abandonment: no email — anonymous session, skipping.', '', '', 0, 'CartAbandonment' );
            return;
        }

        // Build minimal user_data
        $user_data = [
            'email' => Ratul_ACT_Hasher::hash_email( $email ),
        ];
        $user = get_user_by( 'email', $email );
        if ( $user instanceof WP_User && $user->ID > 0 ) {
            $user_data['external_id'] = Ratul_ACT_Identity::get_external_id_for_user( $user->ID );
        }

        $event_id    = Ratul_ACT_Dedup::generate_event_id( 'abandon_' . $session_id );
        $custom_data = [
        ...
        $google_on = get_option( 'ratul_act_google_enabled', 0 );

        if ( $meta_on && Ratul_ACT_Consent::is_granted( 'meta' ) ) {
            $e      = ( new Ratul_ACT_Event( 'InitiateCheckout', $event_id ) )->set_user_data( $user_data )->set_custom_data( $custom_data );
            $result = Ratul_ACT_Meta::send( $e );
            Ratul_ACT_Logger::log( $result['status'] ?? 'error', 'meta', 'Cart abandonment email=' . $email, '', $event_id, 0, 'CartAbandonment' );
        }

        if ( $tiktok_on && Ratul_ACT_Consent::is_granted( 'tiktok' ) ) {
            $e      = ( new Ratul_ACT_Event( 'InitiateCheckout', $event_id ) )->set_user_data( $user_data )->set_custom_data( $custom_data );
            $result = Ratul_ACT_TikTok::send( $e );
            Ratul_ACT_Logger::log( $result['status'] ?? 'error', 'tiktok', 'Cart abandonment email=' . $email, '', $event_id, 0, 'CartAbandonment' );
        }
        ...
        if ( $google_on && Ratul_ACT_Consent::is_granted( 'google' ) ) {
            ...
            Ratul_ACT_Logger::log( $result['status'] ?? 'error', 'google', 'Cart abandonment email=' . $email, '', $event_id, 0, 'CartAbandonment' );
        }
