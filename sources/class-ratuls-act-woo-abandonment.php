...
/**
 * Ratuls_ACT_WooAbandonment  v1.1
 *
 ...
 *      the ratuls_act_pending_abandonments option keyed by session hash.
 *      The record contains the cart snapshot, user_data, and the timestamp
 *      of the last update.
 *
 *   2. A WP-Cron job (ratuls_act_check_abandonments) runs every 15 minutes.
 ...
 *   `ratuls_act_abandonment_window` filter (value in minutes).
 *
 * Options:
 *   ratuls_act_source_abandonment_enabled  — 0/1, default 0 (opt-in)
 *   ratuls_act_abandonment_window_minutes  — int, default 60
 *
 * Changes in v1.1:
 *   NEW-4 (CRITICAL): fire_abandonment_event() now checks
 *     Ratuls_ACT_Consent::is_granted() for each platform before sending.
 ...
 *     same 4-tier priority chain used in Ratuls_ACT_Frontend v2.2:
 ...
 */
class Ratuls_ACT_WooAbandonment {

    /** Option key for the pending abandonment records store. */
    const STORE_KEY = 'ratuls_act_pending_abandonments';

    /** Cron hook name. */
    const CRON_HOOK = 'ratuls_act_check_abandonments';

    public static function init() {
        if ( ! get_option( 'ratuls_act_enabled', 1 ) ) return;
        if ( ! get_option( 'ratuls_act_source_abandonment_enabled', 0 ) ) return;
        ...
        if ( ! wp_next_scheduled( self::CRON_HOOK ) ) {
            wp_schedule_event( time(), 'ratuls_act_15min', self::CRON_HOOK );
        }
    ...
    public static function on_cart_updated() {
        ...
        $event_id = ! empty( $existing['event_id'] )
            ? $existing['event_id']
            : Ratuls_ACT_Dedup::generate_event_id( 'abandon_' . $session_id );
    ...
    public static function process_abandonments() {
         ...
         * Default: 60. Override via `ratuls_act_abandonment_window` filter.
         *
         * @param int $window_minutes
         */
        $window_minutes = (int) apply_filters(
            'ratuls_act_abandonment_window',
            (int) get_option( 'ratuls_act_abandonment_window_minutes', 60 )
        );
        ...
        $meta_on   = (bool) get_option( 'ratuls_act_meta_enabled', 0 );
        $tiktok_on = (bool) get_option( 'ratuls_act_tiktok_enabled', 0 );
        $google_on = (bool) get_option( 'ratuls_act_google_enabled', 0 );
    ...
    private static function fire_abandonment_event(
        ...
        $event_id  = $record['event_id']  ?? Ratuls_ACT_Dedup::generate_event_id();
        ...
        if ( $meta_on && Ratuls_ACT_Consent::is_granted( 'meta' ) ) {
            $e = ( new Ratuls_ACT_Event( 'InitiateCheckout', $event_id ) )
        ...
        if ( $google_on && Ratuls_ACT_Consent::is_granted( 'google' ) ) {
            $e = ( new Ratuls_ACT_Event( 'begin_checkout', $event_id ) )
                ->set_user_data( $user_data )
                ->set_custom_data( $custom_data );
     ...
     * priority chain used in Ratuls_ACT_Frontend v2.2:
    ...
    private static function build_session_user_data(): array {
        ...
        // NEW-5 fix: 4-tier IP chain — mirrors Ratuls_ACT_Frontend::get_request_ip().
        $ip = '';
        ...
        if ( is_user_logged_in() ) {
            ...
            if ( $user->user_email ) {
                $data['email']       = Ratuls_ACT_Hasher::hash_email( $user->user_email );
                $data['external_id'] = Ratuls_ACT_Hasher::hash( (string) $user->ID );
            }
        }
